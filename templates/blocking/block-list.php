...
<div class="sn-block-list" id="sn-block-list" data-page="<?php echo esc_attr( $page ); ?>">
	<h3 class="sn-block-list__title"><?php esc_html_e( 'Blocked Users', 'social-network-6' ); ?></h3>

	<?php if ( empty( $blocks ) ) : ?>
	<p class="sn-block-list__empty"><?php esc_html_e( 'You have not blocked anyone.', 'social-network-6' ); ?></p>
	<?php else : ?>
	...
	<li class="sn-block-list__item" data-user-id="<?php echo esc_attr( $block->blocked_id ); ?>">
		...
		<div class="sn-block-list__info">
			...
			<span class="sn-block-list__date">
				...
				echo esc_html( sprintf(
					...
					__( 'Blocked %s', 'social-network-6' ),
					human_time_diff( strtotime( $block->date_created ) ) . ' ' . __( 'ago', 'social-network-6' )
				) );
				?>
			</span>
		...
		<button class="sn-btn sn-btn--danger-outline sn-unblock-btn"
			...
			<?php esc_html_e( 'Unblock', 'social-network-6' ); ?>
		</button>
	</li>
	<?php endforeach; ?>
	</ul>
	...
	<div class="sn-block-list__pagination">
		...
		<a class="sn-btn sn-btn--secondary"
		   href="<?php echo esc_url( add_query_arg( 'block_page', $page - 1 ) ); ?>">
			<?php esc_html_e( '← Previous', 'social-network-6' ); ?>
		</a>
		<?php endif; ?>
		<?php if ( $has_more ) : ?>
		<a class="sn-btn sn-btn--secondary"
		   href="<?php echo esc_url( add_query_arg( 'block_page', $page + 1 ) ); ?>">
			<?php esc_html_e( 'Next →', 'social-network-6' ); ?>
		</a>
		<?php endif; ?>
	</div>
	<?php endif; ?>
