...
if ( $_group_suspended ) {
	...
	echo '<h2 class="arshid6social-suspended-notice__title">' . esc_html__( 'Group Suspended', 'social-network-6' ) . '</h2>';
	echo '<p class="arshid6social-suspended-notice__msg">' . esc_html__( 'This group has been suspended and its content is not available.', 'social-network-6' ) . '</p>';
	...
	if ( $_viewer_is_admin ) {
		echo '<a href="' . esc_url( admin_url( 'admin.php?page=arshid6social-moderation&moderation_view=groups' ) ) . '" class="arshid6social-btn arshid6social-btn-secondary">' . esc_html__( 'Manage in Admin', 'social-network-6' ) . '</a>';
	}
	echo '</div>';
	echo '</div>';
	echo '</div></div>';
...
$privacy_labels = array(
	'public'  => __( 'Public', 'social-network-6' ),
	'private' => __( 'Private', 'social-network-6' ),
	'hidden'  => __( 'Hidden', 'social-network-6' ),
);
...
if ( $is_admin ) {
	$tabs['manage'] = __( 'Manage', 'social-network-6' );
}
...
<div class="arshid6social-container" style="padding-block:2rem;">
	...
	<div class="arshid6social-card" id="arshid6social-group-header-card">
		...
		<div class="arshid6social-profile-cover"
			...
			printf( esc_attr__( '%s cover photo', 'social-network-6' ), esc_attr( $group['name'] ) ); ?>">
			<?php if ( $is_admin ) : ?>
				<button type="button" class="arshid6social-btn arshid6social-btn-sm arshid6social-btn-secondary arshid6social-cover-edit-btn"
					...
					<?php esc_html_e( 'Edit Cover', 'social-network-6' ); ?>
				</button>
			<?php endif; ?>
		</div>
			...
			<div class="arshid6social-avatar-wrap arshid6social-group-avatar-wrap">
				...
				<?php if ( $is_admin ) : ?>
					<button type="button" class="arshid6social-group-avatar-edit-btn"
						title="<?php esc_attr_e( 'Change group photo', 'social-network-6' ); ?>"
						data-group-id="<?php echo esc_attr( $group_id ); ?>">
						&#9998;
					</button>
				<?php endif; ?>
				...
				<div class="arshid6social-profile-meta">
					<span>
						...
						printf(
							...
							esc_html( _n( '%s member', '%s members', $group['memberCount'] ?? 0, 'social-network-6' ) ),
			...
			<div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
				<?php if ( is_user_logged_in() ) : ?>
					<?php if ( $is_member ) : ?>
						<button type="button" class="arshid6social-btn arshid6social-btn-secondary arshid6social-group-leave-btn"
							...
							<?php esc_html_e( 'Leave Group', 'social-network-6' ); ?>
						</button>
					<?php elseif ( $is_public ) : ?>
						<button type="button" class="arshid6social-btn arshid6social-btn-primary arshid6social-group-join-btn"
							...
							<?php esc_html_e( 'Join Group', 'social-network-6' ); ?>
						</button>
					<?php else : ?>
						<button type="button" class="arshid6social-btn arshid6social-btn-primary arshid6social-group-join-btn"
							data-group-id="<?php echo esc_attr( $group_id ); ?>"
				...
				<?php if ( is_user_logged_in() && ! $is_admin ) : ?>
					<button class="arshid6social-btn arshid6social-btn-ghost arshid6social-report-btn"
						...
						title="<?php esc_attr_e( 'Report this group', 'social-network-6' ); ?>">
		...
		<!-- Tabs -->
		<nav class="arshid6social-tabs" aria-label="<?php esc_attr_e( 'Group sections', 'social-network-6' ); ?>" role="tablist">
	...
	<div class="arshid6social-grid arshid6social-grid--sidebar" style="margin-block-start:1.5rem;">
		...
		<div>
			<div class="arshid6social-card arshid6social-card--about">
				<div class="arshid6social-card__header"><?php esc_html_e( 'About', 'social-network-6' ); ?></div>
				<div class="arshid6social-card__body">
					...
					<dl class="arshid6social-group-about-dl">
						<div>
							<dt><?php esc_html_e( 'Members', 'social-network-6' ); ?></dt>
							<dd><?php echo esc_html( number_format_i18n( $group['memberCount'] ?? 0 ) ); ?></dd>
						</div>
						<div>
							<dt><?php esc_html_e( 'Privacy', 'social-network-6' ); ?></dt>
		...
		<div>
					...
					<?php if ( $is_member && is_user_logged_in() ) : ?>
						<form class="arshid6social-activity-form arshid6social-card"
							...
							<textarea name="content" class="arshid6social-activity-composer"
								placeholder="<?php esc_attr_e( 'Post in this group…', 'social-network-6' ); ?>"
					...
					<?php elseif ( ! is_user_logged_in() ) : ?>
						<div class="arshid6social-card" style="padding:1.25rem;text-align:center;color:var(--arshid6social-text-muted);">
							<a href="<?php echo esc_url( wp_login_url( $group['url'] ?? '' ) ); ?>" class="arshid6social-btn arshid6social-btn-primary" style="margin-top:.5rem;">
								<?php esc_html_e( 'Log in to post', 'social-network-6' ); ?>
							</a>
						</div>
					<?php endif; ?>
					<div class="arshid6social-activity-feed" aria-label="<?php esc_attr_e( 'Group activity', 'social-network-6' ); ?>"></div>
			...
			<?php elseif ( 'members' === $current_tab ) : ?>
				...
				<div class="arshid6social-group-members-grid">
					...
					if ( empty( $rows ) ) :
						?>
						<p class="arshid6social-text-muted" style="color:var(--arshid6social-text-muted);"><?php esc_html_e( 'No members yet.', 'social-network-6' ); ?></p>
					<?php else : ?>
						...
						<div class="arshid6social-member-card arshid6social-card">
							...
							<div class="arshid6social-member-card-body">
								...
								<?php if ( $row->is_admin ) : ?>
									<span class="arshid6social-status-badge arshid6social-status-active"><?php esc_html_e( 'Admin', 'social-network-6' ); ?></span>
								<?php endif; ?>
							</div>
						</div>
						<?php endforeach; ?>
			...
			<?php if ( 'manage' === $current_tab && $is_admin ) : ?>
				...
				<div class="arshid6social-card" style="padding:2rem;margin-bottom:1.5rem;">
					<h2 style="margin:0 0 1.25rem;font-size:1.1rem;font-weight:600;"><?php esc_html_e( 'Edit Group', 'social-network-6' ); ?></h2>
					<form class="arshid6social-edit-group-form" novalidate>
						...
						<div class="arshid6social-form-group">
							...
							<select id="arshid6social-edit-group-status" name="status" class="arshid6social-input">
								<option value="public"  <?php selected( $group['status'], 'public' ); ?>><?php esc_html_e( 'Public — anyone can join and see posts', 'social-network-6' ); ?></option>
								<option value="private" <?php selected( $group['status'], 'private' ); ?>><?php esc_html_e( 'Private — anyone can request, but only members see posts', 'social-network-6' ); ?></option>
								<option value="hidden"  <?php selected( $group['status'], 'hidden' ); ?>><?php esc_html_e( 'Hidden — invite only, not listed in directory', 'social-network-6' ); ?></option>
							</select>
						</div>
						...
						<div class="arshid6social-form-actions">
							<button type="submit" class="arshid6social-btn arshid6social-btn-primary">
								<?php esc_html_e( 'Save Changes', 'social-network-6' ); ?>
							</button>
						</div>
					</form>
				</div>
				...
				<div class="arshid6social-card" style="padding:2rem;border:1px solid var(--arshid6social-danger,#dc2626);">
					<h2 style="margin:0 0 .5rem;font-size:1.1rem;font-weight:600;color:var(--arshid6social-danger,#dc2626);"><?php esc_html_e( 'Delete Group', 'social-network-6' ); ?></h2>
					<p style="margin:0 0 1rem;color:var(--arshid6social-text-muted);"><?php esc_html_e( 'Permanently delete this group and all its data. This cannot be undone.', 'social-network-6' ); ?></p>
					<button type="button" class="arshid6social-btn arshid6social-btn-danger arshid6social-group-delete-btn"
						...
						<?php esc_html_e( 'Delete Group', 'social-network-6' ); ?>
					</button>
				</div>
				...
				( function () {
					...
					if ( editForm ) {
						editForm.addEventListener( 'submit', async ( e ) => {
							...
							btn.disabled = true;
							btn.textContent = '<?php echo esc_js( __( 'Saving…', 'social-network-6' ) ); ?>';
							...
							try {
								...
								if ( data.success ) {
									if ( msgBox ) {
										msgBox.textContent = data.data?.message || '<?php echo esc_js( __( 'Saved.', 'social-network-6' ) ); ?>';
										msgBox.classList.add( 'arshid6social-notice--success' );
										msgBox.hidden = false;
									}
								} else {
									if ( msgBox ) {
										msgBox.textContent = data.data?.message || '<?php echo esc_js( __( 'Could not save.', 'social-network-6' ) ); ?>';
								...
								if ( msgBox ) {
									msgBox.textContent = '<?php echo esc_js( __( 'Network error. Please try again.', 'social-network-6' ) ); ?>';
									msgBox.classList.add( 'arshid6social-notice--error' );
									msgBox.hidden = false;
								}
							} finally {
								btn.disabled = false;
								btn.textContent = '<?php echo esc_js( __( 'Save Changes', 'social-network-6' ) ); ?>';
							}
						} );
					}
					...
					if ( delBtn ) {
						delBtn.addEventListener( 'click', async () => {
							...
							if ( ! window.confirm( '<?php echo esc_js( __( 'Are you sure you want to delete this group? This cannot be undone.', 'social-network-6' ) ); ?>' ) ) {
								return;
							}
							delBtn.disabled = true;
							delBtn.textContent = '<?php echo esc_js( __( 'Deleting…', 'social-network-6' ) ); ?>';
							...
							try {
								...
								if ( data.success ) {
									...
									alert( data.data?.message || '<?php echo esc_js( __( 'Could not delete group.', 'social-network-6' ) ); ?>' );
									delBtn.disabled = false;
									delBtn.textContent = '<?php echo esc_js( __( 'Delete Group', 'social-network-6' ) ); ?>';
								}
							} catch {
								alert( '<?php echo esc_js( __( 'Network error. Please try again.', 'social-network-6' ) ); ?>' );
								delBtn.disabled = false;
								delBtn.textContent = '<?php echo esc_js( __( 'Delete Group', 'social-network-6' ) ); ?>';
							}
						} );
