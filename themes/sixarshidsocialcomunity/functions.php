...
add_action( 'after_setup_theme', function () {
	...
	register_nav_menus( array(
		'socialnetworksix-primary' => __( 'Primary Navigation', 'social-network-6' ),
	) );
} );
...
add_shortcode( 'a6sc_activity', function () {
	if ( ! shortcode_exists( 'arshid6social_activity' ) ) {
		return '<div class="socialnetworksix-feed__placeholder" style="padding:32px 16px;color:#536471;text-align:center;">'
			. esc_html__( 'Activity feed loads here.', 'social-network-6' )
			. '</div>';
	}
	try {
...
add_shortcode( 'a6sc_story', function () {
	...
	return '<div class="socialnetworksix-feed__placeholder" style="padding:24px 16px;color:#536471;text-align:center;">'
		. esc_html__( 'Stories are turned off.', 'social-network-6' )
		. '</div>';
} );
...
add_shortcode( 'a6sc_new_activity', function () {
	if ( ! shortcode_exists( 'arshid6social_activity' ) ) {
		return '<div class="socialnetworksix-feed__placeholder" style="padding:32px 16px;color:#536471;text-align:center;">'
			. esc_html__( 'Post composer loads here.', 'social-network-6' )
			. '</div>';
	}
	try {
...
add_shortcode( 'a6sc_compose_button', function () {
	...
	return '<a href="' . esc_url( (string) $url ) . '" class="a6sc-compose-btn">'
		...
		. '<span class="a6sc-compose-btn__label">' . esc_html__( 'Post', 'social-network-6' ) . '</span>'
		. '</a>';
} );
...
add_shortcode( 'a6sc_user_menu', function () {
	if ( ! is_user_logged_in() ) {
		...
		return '<a href="' . esc_url( (string) $login_url ) . '" class="a6sc-user-menu a6sc-user-menu--guest">'
			. esc_html__( 'Log in', 'social-network-6' ) . '</a>';
	}
	...
	return '<div class="a6sc-user-menu">'
		...
		. '</span>'
		. '</a>'
		. '<a href="' . esc_url( $logout_url ) . '" class="a6sc-user-menu__logout" title="' . esc_attr__( 'Log out', 'social-network-6' ) . '">' . a6sc_svg( 'exit' ) . '</a>'
		. '</div>';
} );
...
add_shortcode( 'a6sc_theme_toggle', function () {
	return '<button class="a6sc-theme-toggle" aria-label="' . esc_attr__( 'Toggle dark mode', 'social-network-6' ) . '">'
...
function a6sc_nav_items(): array {
	...
	return array(
		array( 'page_id' => $front_id,  'fallback' => 'home',     'label' => __( 'Home',          'social-network-6' ), 'url' => home_url( '/' ) ),
		array( 'page_id' => $explore_id,'fallback' => 'explore',  'label' => __( 'Explore',       'social-network-6' ), 'url' => $explore_id  ? get_permalink( $explore_id )  : home_url( '/members/' )     ),
		array( 'page_id' => $notif_id,  'fallback' => 'bell',     'label' => __( 'Notifications', 'social-network-6' ), 'url' => $notif_id    ? get_permalink( $notif_id )    : home_url( '/notifications/' ), 'badge_id' => 'arshid6social-notification-count' ),
		array( 'page_id' => $msg_id,    'fallback' => 'message',  'label' => __( 'Messages',      'social-network-6' ), 'url' => $msg_id      ? get_permalink( $msg_id )      : home_url( '/messages/' ),     'badge_id' => 'arshid6social-messages-count'     ),
		array( 'page_id' => $saved_id,  'fallback' => 'bookmark', 'label' => __( 'Saved',         'social-network-6' ), 'url' => $saved_id    ? get_permalink( $saved_id )    : home_url( '/saved/' )       ),
		array( 'page_id' => $groups_id, 'fallback' => 'users',    'label' => __( 'Groups',        'social-network-6' ), 'url' => $groups_id   ? get_permalink( $groups_id )   : home_url( '/groups/' )      ),
		array( 'page_id' => $market_id, 'fallback' => 'verified', 'label' => __( 'Marketplace',   'social-network-6' ), 'url' => $market_id   ? get_permalink( $market_id )   : home_url( '/marketplace/' ) ),
		array( 'page_id' => 0,          'fallback' => 'profile',  'label' => __( 'Profile',       'social-network-6' ), 'url' => $profile_url ),
	);
}
...
add_shortcode( 'a6sc_sidebar_nav', function () {
	...
	$out = '<nav class="a6sc-sidenav" aria-label="' . esc_attr__( 'Primary Navigation', 'social-network-6' ) . '">';
...
function a6sc_render_page_nav( string $menu = 'socialnetworksix-primary', bool $in_editor = false ): string {
	...
	if ( empty( $items ) ) {
		return '<nav class="a6sc-sidenav a6sc-sidenav--empty"><p style="padding:12px;color:#888;font-size:13px;">'
			. esc_html__( 'No pages found. Create a Page (or assign a menu to “Primary Navigation”) to populate this list.', 'social-network-6' )
			. '</p></nav>';
	}
...
add_action( 'init', function () {
	register_block_type(
		...
		array(
			...
			'category'        => 'theme',
			'icon'            => 'menu',
			'description'     => __( 'Lists your Pages with their Page Icons. Change an icon in Pages → edit → Page Icon.', 'social-network-6' ),
			'attributes'      => array(
				'menu' => array(
					'type'    => 'string',
		...
		array(
			...
			'callback'            => function ( $request ) {
				...
				if ( ! $page_id || ! current_user_can( 'edit_post', $page_id ) ) {
					return new WP_Error( 'a6sc_forbidden', __( 'You cannot edit this page.', 'social-network-6' ), array( 'status' => 403 ) );
				}
...
add_action( 'admin_menu', function () {
	add_theme_page(
		__( 'Sidebar Navigation', 'social-network-6' ),
		__( 'Sidebar Navigation', 'social-network-6' ),
		'edit_pages',
		'a6sc-sidebar-nav',
...
function a6sc_render_nav_admin_page() {
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Sidebar Navigation', 'social-network-6' ); ?></h1>
		<p class="description">
			<?php esc_html_e( 'Drag ⠿ (or use ▲▼) to reorder the pages shown in the left sidebar, and click ✎ to change each page’s icon. Changes save automatically.', 'social-network-6' ); ?>
		</p>
		<div id="a6sc-nav-admin" class="a6sc-nav-admin">
			<p><span class="spinner is-active" style="float:none;"></span> <?php esc_html_e( 'Loading pages…', 'social-network-6' ); ?></p>
		</div>
	</div>
	<?php
}
...
add_action( 'admin_enqueue_scripts', function ( $hook ) {
	...
	wp_localize_script( 'a6sc-page-nav-admin', 'a6scPageNav', array(
		...
		'nonce'    => wp_create_nonce( 'wp_rest' ),
		'i18n'     => array(
			'reorder'  => __( 'Drag ⠿ (or use ▲▼) to reorder · click ✎ to change a page’s icon.', 'social-network-6' ),
			'noPages'  => __( 'No pages found. Create a Page first.', 'social-network-6' ),
			'pickIcon' => __( 'Select an Icon', 'social-network-6' ),
			'search'   => __( 'Search icons…', 'social-network-6' ),
			'more'     => __( 'Showing first 150 — keep typing to narrow results.', 'social-network-6' ),
			'moveUp'   => __( 'Move up', 'social-network-6' ),
			'moveDown' => __( 'Move down', 'social-network-6' ),
			'edit'     => __( 'Change icon', 'social-network-6' ),
...
add_shortcode( 'a6sc_bottom_nav', function () {
	$current = trailingslashit( esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ?? '' ) ) );
	$out = '<nav class="a6sc-bnav" aria-label="' . esc_attr__( 'Mobile Navigation', 'social-network-6' ) . '">';
...
add_shortcode( 'a6sc_right_sidebar', function () {
	...
	if ( $who_to_follow ) {
		...
		$panel = '<div class="wp-block-group a6sc-panel">'
			. '<h2 class="wp-block-heading a6sc-panel__title">' . esc_html__( 'Who to follow', 'social-network-6' ) . '</h2>'
			. $who_to_follow
			. '<p class="a6sc-panel__show-more"><a href="' . esc_url( $members_url ) . '">' . esc_html__( 'View all members →', 'social-network-6' ) . '</a></p>'
			. '</div>';
	}
