...
function a6sc_flush_templates() {
	$ver = 'v3.0.3';
	if ( get_option( 'a6sc_tpl_ver' ) === $ver ) {
		return;
	}
	...
	update_option( 'a6sc_tpl_ver', $ver );
}
 ...
 * Wraps the plugin's [arshid6social_stories_tray] (only registered when Stories are enabled).
 */
add_shortcode(
	'a6sc_story',
	function () {
		if ( shortcode_exists( 'arshid6social_stories_tray' ) ) {
			try {
				return wp_kses_post( do_shortcode( '[arshid6social_stories_tray]' ) );
			} catch ( \Throwable $e ) {
				return '';
			}
		}
...
function a6sc_nav_items(): array {
	...
	return array(
		array(
			'page_id'  => $front_id,
			'fallback' => 'home',
			'label'    => __( 'Home', '6arshid-social-community' ),
			'url'      => home_url( '/' ),
		),
		array(
			'page_id'  => $explore_id,
			'fallback' => 'explore',
			'label'    => __( 'Explore', '6arshid-social-community' ),
			'url'      => $explore_id ? get_permalink( $explore_id ) : home_url( '/members/' ),
		),
		array(
			'page_id'  => $notif_id,
			'fallback' => 'bell',
			'label'    => __( 'Notifications', '6arshid-social-community' ),
			'url'      => $notif_id ? get_permalink( $notif_id ) : home_url( '/notifications/' ),
			'badge_id' => 'arshid6social-notification-count',
		),
		array(
			'page_id'  => $msg_id,
			'fallback' => 'message',
			'label'    => __( 'Messages', '6arshid-social-community' ),
			'url'      => $msg_id ? get_permalink( $msg_id ) : home_url( '/messages/' ),
			'badge_id' => 'arshid6social-messages-count',
		),
		array(
			'page_id'  => $saved_id,
			'fallback' => 'bookmark',
			'label'    => __( 'Saved', '6arshid-social-community' ),
			'url'      => $saved_id ? get_permalink( $saved_id ) : home_url( '/saved/' ),
		),
		array(
			'page_id'  => $groups_id,
			'fallback' => 'users',
			'label'    => __( 'Groups', '6arshid-social-community' ),
			'url'      => $groups_id ? get_permalink( $groups_id ) : home_url( '/groups/' ),
		),
		array(
			'page_id'  => $market_id,
			'fallback' => 'verified',
			'label'    => __( 'Marketplace', '6arshid-social-community' ),
			'url'      => $market_id ? get_permalink( $market_id ) : home_url( '/marketplace/' ),
		),
		array(
			'page_id'  => 0,
			'fallback' => 'profile',
			'label'    => __( 'Profile', '6arshid-social-community' ),
			'url'      => $profile_url,
		),
	);
}
...
add_shortcode(
	...
	function () {
		...
		return wp_kses_post(
			'<aside class="socialnetworksix-sidebar">'
			. $logo
			. $nav
			. $compose
