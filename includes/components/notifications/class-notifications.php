...
class Notifications {
	...
	private function maybe_send_email( int $notification_id, array $args ): void {
		...
		$type_info  = self::TYPES[ $args['component_action'] ?? '' ] ?? array( 'label' => __( 'Notification', '6arshid-social-community-main' ) );
		...
		$body = "<!DOCTYPE html><html><body style='font-family:sans-serif;color:#111;max-width:600px;margin:auto;padding:24px;'>
			...
			. esc_html__( 'View all notifications', '6arshid-social-community-main' )
			. "</a></p>
		</body></html>";
	...
	public function format_notification( object $notification ): array {
		...
		$sender_name  = $sender ? esc_html( $sender->display_name ) : __( 'Someone', '6arshid-social-community-main' );
	...
	private function get_description( object $notification ): string {
		...
		$username = $user ? esc_html( $user->display_name ) : __( 'Someone', '6arshid-social-community-main' );
		...
		return match ( $notification->component_action ) {
			...
			'friend_request'        => sprintf( __( '%s sent you a friend request.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'friendship_accepted'   => sprintf( __( '%s accepted your friend request.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'activity_mention'      => sprintf( __( '%s mentioned you in a post.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'group_invitation'      => sprintf( __( '%s invited you to join a group.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'new_message'           => sprintf( __( '%s sent you a message.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'activity_reaction'     => sprintf( __( '%s reacted to your post.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'activity_comment'      => sprintf( __( '%s commented on your post.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'comment_reply'         => sprintf( __( '%s replied to your comment.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'new_follower'          => sprintf( __( '%s started following you.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'story_reaction'        => sprintf( __( '%s reacted to your story.', '6arshid-social-community-main' ), $link ),
			/* translators: %s: linked username */
			'story_reply'           => sprintf( __( '%s replied to your story.', '6arshid-social-community-main' ), $link ),
			'verification_approved' => __( 'Your verification request has been approved.', '6arshid-social-community-main' ),
			'verification_rejected' => __( 'Your verification request has been rejected.', '6arshid-social-community-main' ),
	...
	public function ajax_delete_notification(): void {
		...
		if ( ! $id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid notification.', '6arshid-social-community-main' ) ), 400 );
		}
		...
		if ( $deleted ) {
			...
			wp_send_json_error( array( 'message' => __( 'Could not delete notification.', '6arshid-social-community-main' ) ), 403 );
		}
	}
	...
	public function ajax_save_prefs(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Preferences saved.', '6arshid-social-community-main' ) ) );
	}
	...
	private function send_digest( string $period ): void {
		...
		$subject_tpl = sprintf( __( 'Your %s notifications digest', '6arshid-social-community-main' ), get_bloginfo( 'name' ) );
	...
	private function build_digest_email( \WP_User $user, array $notifications ): string {
		...
		return "<!DOCTYPE html><html><body style='font-family:sans-serif;max-width:600px;margin:auto;color:#111;'>
			...
			<div style='padding:24px;background:#fff;border:1px solid #e5e7eb;border-top:none;'>
				...
				sprintf( esc_html__( 'Hello %s,', '6arshid-social-community-main' ), esc_html( $user->display_name ) ) . "</p>
				<p>" . esc_html__( 'Here are your recent notifications:', '6arshid-social-community-main' ) . "</p>
