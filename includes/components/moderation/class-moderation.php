...
class Moderation {
	...
	public function ajax_submit_report(): void {
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => __( 'You must be logged in to report.', '6arshid-social-community-main' ) ), 401 );
		}
		...
		if ( ! $item_id || ! in_array( $item_type, array( 'profile', 'group' ), true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid report target.', '6arshid-social-community-main' ) ), 400 );
		}

		if ( ! $reason ) {
			wp_send_json_error( array( 'message' => __( 'Please select a reason.', '6arshid-social-community-main' ) ), 400 );
		}

		// Cannot report yourself.
		if ( 'profile' === $item_type && (int) $item_id === get_current_user_id() ) {
			wp_send_json_error( array( 'message' => __( 'You cannot report yourself.', '6arshid-social-community-main' ) ), 400 );
		}
		...
		if ( (bool) get_option( 'arshid6social_report_allow_attachments', false ) && ! empty( $_FILES['attachment']['name'] ) ) {
			...
			if ( ! in_array( $file_mime, $allowed_mime, true ) ) {
				wp_send_json_error( array( 'message' => __( 'Only image files are allowed as attachments.', '6arshid-social-community-main' ) ), 400 );
			}

			$max_size = (int) get_option( 'arshid6social_max_upload_size_mb', 5 ) * 1024 * 1024;
			if ( (int) ( $_FILES['attachment']['size'] ?? 0 ) > $max_size ) {
				wp_send_json_error( array( 'message' => __( 'Attachment file is too large.', '6arshid-social-community-main' ) ), 400 );
			}
		...
		if ( ! $report_id ) {
			wp_send_json_error( array( 'message' => __( 'You have already reported this. It is pending review.', '6arshid-social-community-main' ) ) );
		}

		wp_send_json_success( array( 'message' => __( 'Thank you. Your report has been submitted for review.', '6arshid-social-community-main' ) ) );
	}
	...
	public function handle_blocked_file_redirect(): void {
		...
		echo '<h2 class="arshid6social-suspended-notice__title">' . esc_html__( 'Content Not Available', '6arshid-social-community-main' ) . '</h2>';
		echo '<p class="arshid6social-suspended-notice__text">' . esc_html__( 'This content belongs to a suspended account and is not available.', '6arshid-social-community-main' ) . '</p>';
	...
	public function ajax_suspend_group(): void {
		...
		wp_send_json_success(
			array(
				'suspended' => $new_state,
				'label'     => $new_state
					? __( 'Unsuspend Group', '6arshid-social-community-main' )
					: __( 'Suspend Group', '6arshid-social-community-main' ),
			)
		);
	}
	...
	public function block_suspended_users(): void {
		...
		add_action( 'wp_footer', function () {
			echo '<div class="arshid6social-suspension-notice" style="position:fixed;bottom:0;left:0;right:0;background:#dc2626;color:#fff;padding:12px;text-align:center;z-index:9999;">'
				. esc_html__( 'Your account has been suspended. Please contact an administrator.', '6arshid-social-community-main' )
				. '</div>';
		} );
	}
