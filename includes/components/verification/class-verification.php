...
class Verification {
	...
	public function request_more_info( int $request_id, string $message ): bool {
		...
		if ( $user ) {
			wp_mail(
				$user->user_email,
				__( 'More information needed for your verification request', 'social-network-6' ),
	...
	public function shortcode_request_form( array $atts ): string {
		if ( ! is_user_logged_in() ) {
			return '<p>' . esc_html__( 'You must be logged in to request verification.', 'social-network-6' ) . '</p>';
		}
	...
	public function ajax_submit_request(): void {
		...
		if ( ! arshid6social_check_rate_limit( 'arshid6social_rl_verify', $user_id, (int) get_option( 'arshid6social_verification_rate_limit', 3 ) ) ) {
			wp_send_json_error( array( 'message' => __( 'Too many verification requests. Please try later.', 'social-network-6' ) ), 429 );
		}
		...
		if ( ! empty( $_FILES['document']['tmp_name'] ) ) {
			...
			wp_send_json_error( array( 'message' => __( 'A document upload is required for verification.', 'social-network-6' ) ) );
		}

		$id = $this->submit_request( $user_id, $type, $fields, $doc_paths );
		if ( ! $id ) {
			wp_send_json_error( array( 'message' => __( 'You already have a pending verification request.', 'social-network-6' ) ) );
		}

		$this->notify_admin_new_request( $user_id );

		wp_send_json_success( array( 'message' => __( 'Your verification request has been submitted. We will review it shortly.', 'social-network-6' ) ) );
	}
	...
	public function ajax_resubmit_request(): void {
		...
		if ( ! $pending || 'more_info' !== $pending->status ) {
			wp_send_json_error( array( 'message' => __( 'No request awaiting additional information.', 'social-network-6' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Your additional information has been submitted. We will review it shortly.', 'social-network-6' ) ) );
	}
