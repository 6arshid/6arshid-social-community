...
class Verification {
	...
	public function request_more_info( int $request_id, string $message ): bool {
		...
		if ( $user ) {
			wp_mail(
				$user->user_email,
				__( 'More information needed for your verification request', '6arshid-social-community-main' ),
	...
	public function shortcode_request_form( array $atts ): string {
		if ( ! is_user_logged_in() ) {
			return '<p>' . esc_html__( 'You must be logged in to request verification.', '6arshid-social-community-main' ) . '</p>';
		}
	...
	public function ajax_submit_request(): void {
		...
		if ( ! arshid6social_check_rate_limit( 'arshid6social_rl_verify', $user_id, (int) get_option( 'arshid6social_verification_rate_limit', 3 ) ) ) {
			wp_send_json_error( array( 'message' => __( 'Too many verification requests. Please try later.', '6arshid-social-community-main' ) ), 429 );
		}
		...
		if ( ! empty( $_FILES['document']['tmp_name'] ) ) {
			...
			wp_send_json_error( array( 'message' => __( 'A document upload is required for verification.', '6arshid-social-community-main' ) ) );
		}

		$id = $this->submit_request( $user_id, $type, $fields, $doc_paths );
		if ( ! $id ) {
			wp_send_json_error( array( 'message' => __( 'You already have a pending verification request.', '6arshid-social-community-main' ) ) );
		}

		$this->notify_admin_new_request( $user_id );

		wp_send_json_success( array( 'message' => __( 'Your verification request has been submitted. We will review it shortly.', '6arshid-social-community-main' ) ) );
	}
	...
	public function ajax_resubmit_request(): void {
		...
		if ( ! $pending || 'more_info' !== $pending->status ) {
			wp_send_json_error( array( 'message' => __( 'No request awaiting additional information.', '6arshid-social-community-main' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Your additional information has been submitted. We will review it shortly.', '6arshid-social-community-main' ) ) );
	}
	...
	private function notify_admin_new_request( int $user_id ): void {
		...
		$subject = sprintf(
			...
			__( '[%1$s] New verification request from %2$s', '6arshid-social-community-main' ),
			get_bloginfo( 'name' ),
			$name
		);
		$body = sprintf(
			/* translators: 1: name, 2: email, 3: admin URL */
			__( "User %1\$s (%2\$s) has submitted a verification request.\n\nReview it in the admin panel:\n%3\$s", '6arshid-social-community-main' ),
			$name,
			$email,
