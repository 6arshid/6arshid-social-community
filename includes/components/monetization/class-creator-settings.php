...
class Creator_Settings {
				...
				form.addEventListener( 'submit', async ( e ) => {
					...
					try {
						...
						if ( json.success ) {
							...
							form.querySelector( '#sixarshidsc-iban-input' ).placeholder = '<?php echo esc_js( __( '— IBAN saved; enter a new value to replace —', '6arshid social community' ) ); ?>';
						} else {
							const msg = ( json.data && json.data.message ) ? json.data.message : '<?php echo esc_js( __( 'Error saving IBAN.', '6arshid social community' ) ); ?>';
							if ( errEl ) { errEl.textContent = msg; errEl.hidden = false; }
						}
					} catch {
						if ( errEl ) { errEl.textContent = '<?php echo esc_js( __( 'Network error. Please try again.', '6arshid social community' ) ); ?>'; errEl.hidden = false; }
					} finally {
	...
	private function render_earnings_card( int $user_id ): void {
		...
		$sixarshidsc_cashout_confirm = esc_js( sprintf( __( 'Request cashout of %1$s %2$s? The admin will transfer it to your IBAN.', '6arshid social community' ), number_format( $available, 2 ), $currency ) );
		?>
		<script>
		(function () {
			...
			btn.addEventListener( 'click', async () => {
				...
				btn.disabled = true;
				if ( msgEl ) msgEl.textContent = '<?php echo esc_js( __( 'Submitting…', '6arshid social community' ) ); ?>';
				...
				try {
					...
					if ( json.success ) {
						...
						if ( msgEl ) {
							...
							msgEl.textContent = json.data?.message || '<?php echo esc_js( __( 'Cashout requested! The admin will process it shortly.', '6arshid social community' ) ); ?>';
						}
					} else {
						btn.disabled = false;
						if ( msgEl ) {
							msgEl.style.color = 'var(--sn-color-danger,#dc2626)';
							msgEl.textContent = json.data?.message || '<?php echo esc_js( __( 'Error. Please try again.', '6arshid social community' ) ); ?>';
						}
					}
				} catch {
					btn.disabled = false;
					if ( msgEl ) {
						msgEl.style.color = 'var(--sn-color-danger,#dc2626)';
						msgEl.textContent = '<?php echo esc_js( __( 'Network error. Please try again.', '6arshid social community' ) ); ?>';
					}
				}
			} );
		})();
