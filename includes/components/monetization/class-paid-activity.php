...
class Paid_Activity {
	...
	public function rest_checkout( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $activity ) {
			return new \WP_Error( 'not_found', __( 'Activity not found.', 'social-network-6' ), array( 'status' => 404 ) );
		}
		if ( 'paid' !== $activity->privacy ) {
			return new \WP_Error( 'not_paid', __( 'This post is not a paid post.', 'social-network-6' ), array( 'status' => 400 ) );
		}
		if ( (int) $activity->user_id === $user_id ) {
			return new \WP_Error( 'owner', __( 'You cannot purchase your own post.', 'social-network-6' ), array( 'status' => 400 ) );
		}
		...
		if ( $price_cents <= 0 ) {
			return new \WP_Error( 'no_price', __( 'This post has no price configured.', 'social-network-6' ), array( 'status' => 400 ) );
		}
		...
		if ( ! empty( $pi['error'] ) ) {
			return new \WP_Error(
				'stripe_error',
				$pi['error']['message'] ?? __( 'Payment provider error.', 'social-network-6' ),
				array( 'status' => 502 )
			);
		}
	...
	public function rest_verify_payment( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $pi_id || ! str_starts_with( $pi_id, 'pi_' ) ) {
			return new \WP_Error( 'bad_pi', __( 'Invalid payment_intent ID.', 'social-network-6' ), array( 'status' => 400 ) );
		}
		...
		if (
			...
			return new \WP_Error( 'metadata_mismatch', __( 'Payment metadata does not match.', 'social-network-6' ), array( 'status' => 403 ) );
		}
