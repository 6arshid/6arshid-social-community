...
class Monetization_Settings {
	...
	public function add_tab( array $tabs ): array {
		$tabs['monetization'] = __( 'Monetization', 'social-network-6' );
		return $tabs;
	}
	...
	public function render(): void {
		...
		<div class="notice notice-error inline" style="margin:16px 0 0">
			<p>
				<strong><?php esc_html_e( 'HTTPS required.', 'social-network-6' ); ?></strong>
				<?php esc_html_e( 'Paid Content & Creator Subscriptions requires an SSL certificate. Please enable HTTPS on your site before accepting payments.', 'social-network-6' ); ?>
			</p>
		</div>
		<?php endif; ?>

		<style>
		.sixarshidsc-section { margin: 24px 0 0; }
		.sixarshidsc-section h3 { border-bottom: 1px solid #ddd; padding-bottom: 8px; margin-bottom: 0; }
		.sixarshidsc-key-set { color: #46b450; font-weight: 600; margin-left: 8px; }
		.sixarshidsc-code { font-family: monospace; background: #f6f7f7; padding: 4px 8px; border-radius: 3px; word-break: break-all; }
		</style>

		<h2><?php esc_html_e( 'Paid Content & Creator Subscriptions', 'social-network-6' ); ?></h2>
		<p class="description">
			<?php esc_html_e( 'Let creators monetize content with X-style monthly subscriptions and pay-per-view posts. Stripe Connect handles creator identity verification and bank payouts — raw bank details are never stored here.', 'social-network-6' ); ?>
		</p>

		<!-- Enable toggle -->
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Enable Monetization', 'social-network-6' ); ?></th>
				<td>
					<label>
						<input type="checkbox" name="sixarshidsc_enabled" value="1"
							<?php checked( get_option( 'sixarshidsc_enabled' ) ); ?> />
						<?php esc_html_e( 'Activate Paid Content & Creator Subscriptions', 'social-network-6' ); ?>
					</label>
					<p class="description">
						<?php esc_html_e( 'A page reload is required after first enabling this feature.', 'social-network-6' ); ?>
					</p>
				</td>
			</tr>
		</table>
		...
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Payment Gateway', 'social-network-6' ); ?></h3>
		</div>
		<p class="description" style="margin-top:8px">
			<?php esc_html_e( 'Additional gateways can be registered via the sixarshidsc_payment_gateways filter for regions where Stripe is unavailable.', 'social-network-6' ); ?>
		</p>
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Active Gateway', 'social-network-6' ); ?></th>
				<td>
					<?php
					/**
					...
					$gateways = (array) apply_filters( 'sixarshidsc_payment_gateways', array(
						'stripe_connect' => __( 'Stripe Connect (recommended)', 'social-network-6' ),
					) );
		...
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Stripe Connect Credentials', 'social-network-6' ); ?></h3>
		</div>
		<p class="description" style="margin-top:8px">
			<?php
			printf(
				wp_kses(
					...
					__( 'Find your API keys in the <a href="%s" target="_blank" rel="noopener noreferrer">Stripe Dashboard → Developers → API keys</a>. The secret key and webhook signing secret are stored <strong>encrypted</strong> and are never exposed to the browser or REST API.', 'social-network-6' ),
					array(
		...
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Mode', 'social-network-6' ); ?></th>
				<td>
					<label>
						<input type="checkbox" name="sixarshidsc_stripe_test_mode" value="1"
							<?php checked( get_option( 'sixarshidsc_stripe_test_mode', true ) ); ?> />
						<?php esc_html_e( 'Test mode — use test keys and no real money is charged', 'social-network-6' ); ?>
					</label>
					<p class="description">
						<?php esc_html_e( 'Uncheck only when you are ready to go live.', 'social-network-6' ); ?>
					</p>
				</td>
			</tr>
		</table>
		...
		<h4 style="margin:20px 0 0 10px;font-size:13px;color:#3c434a;">
			<?php esc_html_e( 'Live Keys', 'social-network-6' ); ?>
		</h4>
		<table class="form-table" role="presentation">
			<?php $this->render_key_row( 'sixarshidsc_stripe_pub_key_live',          __( 'Publishable Key (Live)', 'social-network-6' ),  'pk_live_…', false ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_secret_live',           __( 'Secret Key (Live)', 'social-network-6' ),       'sk_live_…', true ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_webhook_secret_live',   __( 'Webhook Secret (Live)', 'social-network-6' ),   'whsec_…',   true ); ?>
		</table>

		<h4 style="margin:20px 0 0 10px;font-size:13px;color:#3c434a;">
			<?php esc_html_e( 'Test Keys', 'social-network-6' ); ?>
		</h4>
		<table class="form-table" role="presentation">
			<?php $this->render_key_row( 'sixarshidsc_stripe_pub_key_test',          __( 'Publishable Key (Test)', 'social-network-6' ),  'pk_test_…', false ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_secret_test',           __( 'Secret Key (Test)', 'social-network-6' ),       'sk_test_…', true ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_webhook_secret_test',   __( 'Webhook Secret (Test)', 'social-network-6' ),   'whsec_…',   true ); ?>
		</table>

		<!-- Webhook URL notice -->
		<div class="notice notice-info inline" style="margin:16px 0 0">
			<p>
				<strong><?php esc_html_e( 'Stripe webhook endpoint:', 'social-network-6' ); ?></strong><br />
				...
				<span class="description">
					...
					printf(
						wp_kses(
							...
							__( 'Register this URL in <a href="%s" target="_blank" rel="noopener noreferrer">Stripe Dashboard → Webhooks</a>. Listen for: <code>customer.subscription.*</code>, <code>invoice.*</code>, <code>payment_intent.*</code>, <code>account.updated</code>.', 'social-network-6' ),
							array(
								'a'    => array( 'href' => array(), 'target' => array(), 'rel' => array() ),
								'code' => array(),
							)
		...
		<!-- Platform revenue -->
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Platform Revenue', 'social-network-6' ); ?></h3>
		</div>
		<p class="description" style="margin-top:8px">
			<?php esc_html_e( 'These fees are applied as a Stripe Connect application fee and deducted before the creator receives their payout. The site owner is responsible for applicable local tax obligations.', 'social-network-6' ); ?>
		</p>
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row">
					<label for="sixarshidsc_platform_fee_pct">
						<?php esc_html_e( 'Platform Fee %', 'social-network-6' ); ?>
					</label>
				</th>
				<td>
					...
					<p class="description">
						<?php esc_html_e( '0 – 100. For example, 10 means the platform retains 10 % of every transaction.', 'social-network-6' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="sixarshidsc_platform_fee_flat">
						<?php esc_html_e( 'Platform Flat Fee', 'social-network-6' ); ?>
					</label>
				</th>
				<td>
					...
					<p class="description">
						<?php esc_html_e( 'Optional fixed amount per transaction, in addition to the percentage fee.', 'social-network-6' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="sixarshidsc_currency">
						<?php esc_html_e( 'Currency', 'social-network-6' ); ?>
					</label>
				</th>
				<td>
					<?php
					...
					<p class="description">
						<?php esc_html_e( 'Must match your Stripe account\'s settlement currency.', 'social-network-6' ); ?>
					</p>
				</td>
			</tr>
		</table>
		...
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Subscription Pricing', 'social-network-6' ); ?></h3>
		</div>
		<table class="form-table" role="presentation" style="margin-top:0">
			<tr>
				<th scope="row">
					<label for="sixarshidsc_min_sub_price">
						<?php esc_html_e( 'Minimum Subscription Price', 'social-network-6' ); ?>
					</label>
				</th>
				<td>
					...
					<span class="description"><?php echo esc_html( $currency ); ?></span>
					<p class="description">
						<?php esc_html_e( 'The lowest monthly price a creator can set for their subscription tier.', 'social-network-6' ); ?>
					</p>
				</td>
			</tr>
		</table>
	...
	private function render_key_row(
		...
		if ( $is_secret ) {
			...
			$input_placeholder = $is_set
				? __( '— key saved; enter a new value to replace —', 'social-network-6' )
				: $placeholder;
		} else {
		...
		<tr>
			...
			<td>
				...
				/>
				<?php if ( $is_set ) : ?>
					<span class="sixarshidsc-key-set">&#10003; <?php esc_html_e( 'Saved', 'social-network-6' ); ?></span>
				<?php endif; ?>
			</td>
		</tr>
		<?php
