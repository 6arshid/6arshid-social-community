...
class Monetization_Settings {
	...
	public function add_tab( array $tabs ): array {
		$tabs['monetization'] = __( 'Monetization', '6arshid-social-community-main' );
		return $tabs;
	}
	...
	public function render(): void {
		...
		<div class="notice notice-error inline" style="margin:16px 0 0">
			<p>
				<strong><?php esc_html_e( 'HTTPS required.', '6arshid-social-community-main' ); ?></strong>
				<?php esc_html_e( 'Paid Content & Creator Subscriptions requires an SSL certificate. Please enable HTTPS on your site before accepting payments.', '6arshid-social-community-main' ); ?>
			</p>
		</div>
		<?php endif; ?>

		<h2><?php esc_html_e( 'Paid Content & Creator Subscriptions', '6arshid-social-community-main' ); ?></h2>
		<p class="description">
			<?php esc_html_e( 'Let creators monetize content with X-style monthly subscriptions and pay-per-view posts. Stripe Connect handles creator identity verification and bank payouts — raw bank details are never stored here.', '6arshid-social-community-main' ); ?>
		</p>

		<!-- Enable toggle -->
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Enable Monetization', '6arshid-social-community-main' ); ?></th>
				<td>
					<label>
						<input type="checkbox" name="sixarshidsc_enabled" value="1"
							<?php checked( get_option( 'sixarshidsc_enabled' ) ); ?> />
						<?php esc_html_e( 'Activate Paid Content & Creator Subscriptions', '6arshid-social-community-main' ); ?>
					</label>
					<p class="description">
						<?php esc_html_e( 'A page reload is required after first enabling this feature.', '6arshid-social-community-main' ); ?>
					</p>
				</td>
			</tr>
		</table>
		...
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Payment Gateway', '6arshid-social-community-main' ); ?></h3>
		</div>
		<p class="description" style="margin-top:8px">
			<?php esc_html_e( 'Additional gateways can be registered via the sixarshidsc_payment_gateways filter for regions where Stripe is unavailable.', '6arshid-social-community-main' ); ?>
		</p>
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Active Gateway', '6arshid-social-community-main' ); ?></th>
				<td>
					<?php
					/**
					...
					$gateways = (array) apply_filters( 'sixarshidsc_payment_gateways', array(
						'stripe_connect' => __( 'Stripe Connect (recommended)', '6arshid-social-community-main' ),
					) );
		...
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Stripe Connect Credentials', '6arshid-social-community-main' ); ?></h3>
		</div>
		<p class="description" style="margin-top:8px">
			<?php
			printf(
				wp_kses(
					...
					__( 'Find your API keys in the <a href="%s" target="_blank" rel="noopener noreferrer">Stripe Dashboard → Developers → API keys</a>. The secret key and webhook signing secret are stored <strong>encrypted</strong> and are never exposed to the browser or REST API.', '6arshid-social-community-main' ),
					array(
		...
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Mode', '6arshid-social-community-main' ); ?></th>
				<td>
					<label>
						<input type="checkbox" name="sixarshidsc_stripe_test_mode" value="1"
							<?php checked( get_option( 'sixarshidsc_stripe_test_mode', true ) ); ?> />
						<?php esc_html_e( 'Test mode — use test keys and no real money is charged', '6arshid-social-community-main' ); ?>
					</label>
					<p class="description">
						<?php esc_html_e( 'Uncheck only when you are ready to go live.', '6arshid-social-community-main' ); ?>
					</p>
				</td>
			</tr>
		</table>
		...
		<h4 style="margin:20px 0 0 10px;font-size:13px;color:#3c434a;">
			<?php esc_html_e( 'Live Keys', '6arshid-social-community-main' ); ?>
		</h4>
		<table class="form-table" role="presentation">
			<?php $this->render_key_row( 'sixarshidsc_stripe_pub_key_live',          __( 'Publishable Key (Live)', '6arshid-social-community-main' ),  'pk_live_…', false ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_secret_live',           __( 'Secret Key (Live)', '6arshid-social-community-main' ),       'sk_live_…', true ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_webhook_secret_live',   __( 'Webhook Secret (Live)', '6arshid-social-community-main' ),   'whsec_…',   true ); ?>
		</table>

		<h4 style="margin:20px 0 0 10px;font-size:13px;color:#3c434a;">
			<?php esc_html_e( 'Test Keys', '6arshid-social-community-main' ); ?>
		</h4>
		<table class="form-table" role="presentation">
			<?php $this->render_key_row( 'sixarshidsc_stripe_pub_key_test',          __( 'Publishable Key (Test)', '6arshid-social-community-main' ),  'pk_test_…', false ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_secret_test',           __( 'Secret Key (Test)', '6arshid-social-community-main' ),       'sk_test_…', true ); ?>
			<?php $this->render_key_row( 'sixarshidsc_stripe_webhook_secret_test',   __( 'Webhook Secret (Test)', '6arshid-social-community-main' ),   'whsec_…',   true ); ?>
		</table>

		<!-- Webhook URL notice -->
		<div class="notice notice-info inline" style="margin:16px 0 0">
			<p>
				<strong><?php esc_html_e( 'Stripe webhook endpoint:', '6arshid-social-community-main' ); ?></strong><br />
				...
				<span class="description">
					...
					printf(
						wp_kses(
							...
							__( 'Register this URL in <a href="%s" target="_blank" rel="noopener noreferrer">Stripe Dashboard → Webhooks</a>. Listen for: <code>customer.subscription.*</code>, <code>invoice.*</code>, <code>payment_intent.*</code>, <code>account.updated</code>.', '6arshid-social-community-main' ),
							array(
								'a'    => array( 'href' => array(), 'target' => array(), 'rel' => array() ),
								'code' => array(),
							)
		...
		<!-- Platform revenue -->
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Platform Revenue', '6arshid-social-community-main' ); ?></h3>
		</div>
		<p class="description" style="margin-top:8px">
			<?php esc_html_e( 'These fees are applied as a Stripe Connect application fee and deducted before the creator receives their payout. The site owner is responsible for applicable local tax obligations.', '6arshid-social-community-main' ); ?>
		</p>
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row">
					<label for="sixarshidsc_platform_fee_pct">
						<?php esc_html_e( 'Platform Fee %', '6arshid-social-community-main' ); ?>
					</label>
				</th>
				<td>
					...
					<p class="description">
						<?php esc_html_e( '0 – 100. For example, 10 means the platform retains 10 % of every transaction.', '6arshid-social-community-main' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="sixarshidsc_platform_fee_flat">
						<?php esc_html_e( 'Platform Flat Fee', '6arshid-social-community-main' ); ?>
					</label>
				</th>
				<td>
					...
					<p class="description">
						<?php esc_html_e( 'Optional fixed amount per transaction, in addition to the percentage fee.', '6arshid-social-community-main' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="sixarshidsc_currency">
						<?php esc_html_e( 'Currency', '6arshid-social-community-main' ); ?>
					</label>
				</th>
				<td>
					<?php
					...
					<p class="description">
						<?php esc_html_e( 'Must match your Stripe account\'s settlement currency.', '6arshid-social-community-main' ); ?>
					</p>
				</td>
			</tr>
		</table>
		...
		<div class="sixarshidsc-section">
			<h3><?php esc_html_e( 'Subscription Pricing', '6arshid-social-community-main' ); ?></h3>
		</div>
		<table class="form-table" role="presentation" style="margin-top:0">
			<tr>
				<th scope="row">
					<label for="sixarshidsc_min_sub_price">
						<?php esc_html_e( 'Minimum Subscription Price', '6arshid-social-community-main' ); ?>
					</label>
				</th>
				<td>
					...
					<span class="description"><?php echo esc_html( $currency ); ?></span>
					<p class="description">
						<?php esc_html_e( 'The lowest monthly price a creator can set for their subscription tier.', '6arshid-social-community-main' ); ?>
					</p>
				</td>
			</tr>
		</table>
	...
	private function render_key_row(
		...
		if ( $is_secret ) {
			...
			$input_placeholder = $is_set
				? __( '— key saved; enter a new value to replace —', '6arshid-social-community-main' )
				: $placeholder;
		} else {
		...
		<tr>
			...
			<td>
				...
				/>
				<?php if ( $is_set ) : ?>
					<span class="sixarshidsc-key-set">&#10003; <?php esc_html_e( 'Saved', '6arshid-social-community-main' ); ?></span>
				<?php endif; ?>
			</td>
		</tr>
		<?php
