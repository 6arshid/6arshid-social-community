...
class Messages_REST extends \WP_REST_Controller {
	...
	public function create_thread( $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $thread_id ) {
			return new \WP_Error( 'arshid6social_message_error', __( 'Could not create thread.', 'social-network-6' ), array( 'status' => 500 ) );
		}
	...
	public function send_message( $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $message_id ) {
			return new \WP_Error( 'arshid6social_message_error', __( 'Could not send message.', 'social-network-6' ), array( 'status' => 500 ) );
		}
