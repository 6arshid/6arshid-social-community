...
class Groups {
	...
	public function ajax_create_group(): void {
		...
		if ( empty( $name ) ) {
			wp_send_json_error( array( 'message' => __( 'Group name is required.', '6arshid-social-community-main' ) ), 400 );
		}

		$group_id = $this->create( array( 'name' => $name, 'description' => $description, 'status' => $status ) );

		if ( ! $group_id ) {
			wp_send_json_error( array( 'message' => __( 'Failed to create group.', '6arshid-social-community-main' ) ), 500 );
		}

		wp_send_json_success(
			array(
				'group'   => $this->format_group( $this->get_by_id( $group_id ) ),
				'message' => __( 'Group created!', '6arshid-social-community-main' ),
			)
		);
	}
	...
	public function ajax_join_group(): void {
		...
		if ( ! $group ) {
			wp_send_json_error( array( 'message' => __( 'Group not found.', '6arshid-social-community-main' ) ), 404 );
		}

		if ( 'hidden' === $group->status ) {
			wp_send_json_error( array( 'message' => __( 'This group cannot be joined directly.', '6arshid-social-community-main' ) ), 403 );
		}
		...
		wp_send_json_success(
			array(
				...
				'message' => $is_confirmed
					? __( 'You have joined the group.', '6arshid-social-community-main' )
					: __( 'Membership request sent.', '6arshid-social-community-main' ),
			)
		);
	}
	...
	public function ajax_leave_group(): void {
		...
		if ( $this->is_admin( $user_id, $group_id ) ) {
			...
			if ( $admin_count <= 1 ) {
				wp_send_json_error( array( 'message' => __( 'You are the only admin. Assign another admin before leaving.', '6arshid-social-community-main' ) ), 422 );
			}
		}

		$this->remove_member( $group_id, $user_id );
		wp_send_json_success( array( 'message' => __( 'You have left the group.', '6arshid-social-community-main' ) ) );
	}
	...
	public function ajax_invite_to_group(): void {
		...
		if ( ! $this->is_member( $current, $group_id ) && ! current_user_can( 'arshid6social_manage_groups' ) ) {
			wp_send_json_error( array( 'message' => __( 'Only group members can invite others.', '6arshid-social-community-main' ) ), 403 );
		}

		if ( ! get_userdata( $invitee_id ) ) {
			wp_send_json_error( array( 'message' => __( 'User not found.', '6arshid-social-community-main' ) ), 404 );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Invitation sent.', '6arshid-social-community-main' ) ) );
	}
	...
	public function ajax_delete_group(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Group deleted.', '6arshid-social-community-main' ) ) );
	}
	...
	public function ajax_update_group(): void {
		...
		wp_send_json_success(
			array(
				'group'   => $this->format_group( $this->get_by_id( $group_id ) ),
				'message' => __( 'Group updated.', '6arshid-social-community-main' ),
			)
		);
	}
	...
	private function save_group_image( array $file, int $group_id, string $slot ): array|\WP_Error {
		...
		if ( ! in_array( $real_mime, $allowed_mime, true ) ) {
			return new \WP_Error( 'invalid_mime', __( 'Only image files are allowed.', '6arshid-social-community-main' ) );
		}

		$max_bytes = (int) get_option( 'arshid6social_max_upload_size_mb', 5 ) * MB_IN_BYTES;
		if ( $file['size'] > $max_bytes ) {
			return new \WP_Error( 'file_too_large', __( 'File is too large.', '6arshid-social-community-main' ) );
		}
		...
		if ( ! wp_mkdir_p( $dest_dir ) ) {
			return new \WP_Error( 'mkdir_failed', __( 'Could not create upload directory.', '6arshid-social-community-main' ) );
		}
		...
		if ( ! is_uploaded_file( $file['tmp_name'] ) ) {
			return new \WP_Error( 'move_failed', __( 'Failed to save file.', '6arshid-social-community-main' ) );
		}
