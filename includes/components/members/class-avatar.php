...
class Avatar {
	...
	private function process_upload( ?array $file, string $image_type, int $user_id ): array|\WP_Error {
		if ( ! $file || isset( $file['error'] ) && UPLOAD_ERR_OK !== $file['error'] ) {
			return new \WP_Error( 'upload_error', __( 'Upload failed. Please try again.', 'social-network-6' ) );
		}
		...
		if ( $file['size'] > $max_bytes ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'File exceeds the maximum allowed size of %s MB.', 'social-network-6' ),
		...
		if ( ! in_array( $mime, $allowed, true ) ) {
			return new \WP_Error( 'invalid_mime', __( 'Invalid file type. Only JPEG, PNG, GIF, and WebP are allowed.', 'social-network-6' ) );
		}
