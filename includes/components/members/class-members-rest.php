...
class Members_REST extends \WP_REST_Controller {
	...
	public function get_items( $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $component ) {
			return new \WP_Error( 'arshid6social_component_disabled', __( 'Members component is not active.', 'social-network-6' ), array( 'status' => 503 ) );
		}
	...
	public function get_item( $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $user ) {
			return new \WP_Error( 'arshid6social_member_not_found', __( 'Member not found.', 'social-network-6' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! is_array( $fields ) ) {
			return new \WP_Error( 'arshid6social_invalid_data', __( 'Fields must be an object.', 'social-network-6' ), array( 'status' => 400 ) );
		}
		...
		if ( ! empty( $errors ) ) {
			return new \WP_Error( 'arshid6social_validation_error', __( 'Validation failed.', 'social-network-6' ), array( 'status' => 422, 'errors' => $errors ) );
		}
	...
	public function update_item_permissions_check( $request ): bool|\WP_Error {
		if ( ! is_user_logged_in() ) {
			return new \WP_Error( 'arshid6social_unauthenticated', __( 'Authentication required.', 'social-network-6' ), array( 'status' => 401 ) );
		}
		...
		if ( $current_user !== $target_user_id && ! current_user_can( 'arshid6social_manage_members' ) ) {
			return new \WP_Error( 'arshid6social_forbidden', __( 'You can only edit your own profile.', 'social-network-6' ), array( 'status' => 403 ) );
		}
