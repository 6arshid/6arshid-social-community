...
class Members {
	...
	public function ajax_update_profile(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Profile updated.', 'social-network-6' ) ) );
	}
	...
	public function ajax_save_bio(): void {
		...
		if ( ! $field_id ) {
			...
			if ( ! $group_id ) {
				wp_send_json_error( array( 'message' => __( 'Profile field group not found.', 'social-network-6' ) ), 500 );
			}
		...
		if ( ! $field_id ) {
			wp_send_json_error( array( 'message' => __( 'Could not create bio field.', 'social-network-6' ) ), 500 );
		}
	...
	public function ajax_save_user_setting(): void {
		...
		if ( ! array_key_exists( $key, $allowed ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid setting.', 'social-network-6' ) ) );
		}

		if ( ! in_array( $value, $allowed[ $key ], true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid value.', 'social-network-6' ) ) );
		}

		update_user_meta( get_current_user_id(), $key, $value );
		wp_send_json_success( array( 'message' => __( 'Saved.', 'social-network-6' ) ) );
	}
	...
	public function ajax_change_password(): void {
		...
		if ( '' === $current || '' === $new || '' === $confirm ) {
			wp_send_json_error( array( 'message' => __( 'All fields are required.', 'social-network-6' ) ) );
		}

		if ( $new !== $confirm ) {
			wp_send_json_error( array( 'message' => __( 'New passwords do not match.', 'social-network-6' ) ) );
		}

		if ( strlen( $new ) < 8 ) {
			wp_send_json_error( array( 'message' => __( 'Password must be at least 8 characters.', 'social-network-6' ) ) );
		}

		$user = wp_get_current_user();
		if ( ! wp_check_password( $current, $user->user_pass, $user->ID ) ) {
			wp_send_json_error( array( 'message' => __( 'Current password is incorrect.', 'social-network-6' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Password changed successfully.', 'social-network-6' ) ) );
	}
	...
	public static function validate_username_restrictions( string $username ): \WP_Error {
		...
		if ( mb_strlen( $username ) < $min_len ) {
			$errors->add(
				...
				sprintf( __( 'Username must be at least %d characters.', 'social-network-6' ), $min_len )
			);
		}
		...
		if ( '' !== $reserved_raw ) {
			...
			if ( in_array( mb_strtolower( $username ), $reserved, true ) ) {
				$errors->add( 'username_reserved', __( 'That username is not available.', 'social-network-6' ) );
			}
		}
	...
	public function ajax_check_username(): void {
		...
		if ( ! $username ) {
			wp_send_json_error( array( 'message' => __( 'Please enter a username.', 'social-network-6' ) ) );
		}
		...
		if ( username_exists( $username ) ) {
			wp_send_json_error( array( 'message' => __( 'That username is already taken.', 'social-network-6' ) ) );
		}

		wp_send_json_success( array( 'message' => __( 'Username is available!', 'social-network-6' ) ) );
	}
	...
	public function ajax_check_username_change(): void {
		...
		if ( ! preg_match( '/^[a-zA-Z0-9_.\-]+$/', $username ) ) {
			wp_send_json_error( array( 'message' => __( 'Username may only contain letters, numbers, underscores, dots and hyphens.', 'social-network-6' ) ) );
		}
		...
		if ( $username === $current_user->user_login ) {
			wp_send_json_success( array( 'message' => __( 'That is your current username.', 'social-network-6' ), 'is_current' => true ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Username is available!', 'social-network-6' ), 'is_current' => false ) );
	}
	...
	public function ajax_change_username(): void {
		...
		if ( mb_strlen( $new_username ) < 3 ) {
			wp_send_json_error( array( 'message' => __( 'Username must be at least 3 characters.', 'social-network-6' ) ) );
		}
		...
		if ( $new_username === $current_user->user_login ) {
			wp_send_json_error( array( 'message' => __( 'That is already your username.', 'social-network-6' ) ) );
		}
		...
		if ( false === $updated ) {
			wp_send_json_error( array( 'message' => __( 'Could not update username. Please try again.', 'social-network-6' ) ) );
		}
		...
		wp_send_json_success(
			array(
				'message'     => __( 'Username changed successfully!', 'social-network-6' ),
	...
	public function ajax_save_display_name(): void {
		...
		if ( '' === $display_name ) {
			wp_send_json_error( array( 'message' => __( 'Display name cannot be empty.', 'social-network-6' ) ) );
		}

		if ( mb_strlen( $display_name ) > 100 ) {
			wp_send_json_error( array( 'message' => __( 'Display name is too long.', 'social-network-6' ) ) );
		}
	...
	public function ajax_save_friends_privacy(): void {
		...
		if ( ! in_array( $privacy, array( 'public', 'private' ), true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid privacy value.', 'social-network-6' ) ) );
		}
	...
	public function register_data_exporter( array $exporters ): array {
		$exporters[] = array(
			'exporter_friendly_name' => __( '6Arshid Social Community Profile Data', 'social-network-6' ),
	...
	public function register_data_eraser( array $erasers ): array {
		$erasers[] = array(
			'eraser_friendly_name' => __( '6Arshid Social Community Profile Data', 'social-network-6' ),
