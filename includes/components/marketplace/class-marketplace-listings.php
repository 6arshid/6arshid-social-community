...
class Marketplace_Listings {
	...
	private static function time_ago( string $datetime ): string {
		...
		if ( $diff < 60 )       return __( 'just now', '6arshid-social-community-main' );
		/* translators: %d: number of minutes */
		if ( $diff < 3600 )     return sprintf( _n( '%d minute ago', '%d minutes ago', (int) ( $diff / 60 ),   '6arshid-social-community-main' ), (int) ( $diff / 60 ) );
		/* translators: %d: number of hours */
		if ( $diff < 86400 )    return sprintf( _n( '%d hour ago',   '%d hours ago',   (int) ( $diff / 3600 ), '6arshid-social-community-main' ), (int) ( $diff / 3600 ) );
		/* translators: %d: number of days */
		if ( $diff < 604800 )   return sprintf( _n( '%d day ago',    '%d days ago',    (int) ( $diff / 86400 ),'6arshid-social-community-main' ), (int) ( $diff / 86400 ) );
	...
	public function ajax_upload_photo(): void {
		...
		if ( ! $token ) {
			wp_send_json_error( array( 'message' => __( 'Invalid form session.', '6arshid-social-community-main' ) ), 400 );
		}
		...
		if ( count( $draft ) >= $max ) {
			...
			wp_send_json_error( array( 'message' => sprintf( __( 'Maximum %d photos allowed.', '6arshid-social-community-main' ), $max ) ), 400 );
		}

		if ( empty( $_FILES['photo'] ) || UPLOAD_ERR_OK !== $_FILES['photo']['error'] ) {
			wp_send_json_error( array( 'message' => __( 'No valid file received.', '6arshid-social-community-main' ) ), 400 );
		}
		...
		if ( $file['size'] > $max_mb * MB_IN_BYTES ) {
			...
			wp_send_json_error( array( 'message' => sprintf( __( 'File exceeds the %d MB limit.', '6arshid-social-community-main' ), $max_mb ) ), 400 );
		}
		...
		if ( ! in_array( $mime, $allowed, true ) ) {
			wp_send_json_error( array( 'message' => __( 'Only JPEG, PNG, WebP, and GIF images are accepted.', '6arshid-social-community-main' ) ), 400 );
		}
	...
	public function ajax_save_listing(): void {
		...
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => __( 'You must be logged in to post a listing.', '6arshid-social-community-main' ) ), 401 );
		}
		...
		if ( get_option( 'arshid6social_marketplace_require_verified', false ) ) {
			...
			if ( $verification && ! $verification->is_verified( $user_id ) ) {
				wp_send_json_error( array( 'message' => __( 'Only verified users can post listings.', '6arshid-social-community-main' ) ), 403 );
			}
		}
		...
		if ( $is_publish ) {
			...
			if ( $daily_max > 0 && ! arshid6social_check_rate_limit( 'arshid6social_mkt_new', $user_id, $daily_max ) ) {
				wp_send_json_error( array( 'message' => __( 'You have reached your daily listing limit. Try again tomorrow.', '6arshid-social-community-main' ) ), 429 );
			}
			...
			if ( $max_active > 0 ) {
				...
				if ( $active_count >= $max_active ) {
					wp_send_json_error( array(
						...
						'message' => sprintf( __( 'You can have a maximum of %d active listings at once.', '6arshid-social-community-main' ), $max_active ),
					), 400 );
				}
			}
		}
		...
		// ── Validate ─────────────────────────────────────────────────────────
		if ( '' === $title ) {
			wp_send_json_error( array( 'message' => __( 'Title is required.', '6arshid-social-community-main' ) ), 400 );
		}
		if ( mb_strlen( $title ) > 200 ) {
			wp_send_json_error( array( 'message' => __( 'Title must be 200 characters or less.', '6arshid-social-community-main' ) ), 400 );
		}
		...
		if ( $banned_raw ) {
			...
			foreach ( $banned_words as $word ) {
				if ( $word && str_contains( $check_text, $word ) ) {
					wp_send_json_error( array( 'message' => __( 'Your listing contains prohibited content and cannot be posted.', '6arshid-social-community-main' ) ), 400 );
				}
			}
		}
		...
		if ( ! $ok ) {
			wp_send_json_error( array( 'message' => __( 'Failed to save your listing. Please try again.', '6arshid-social-community-main' ) ), 500 );
		}
		...
		if ( 'pending' === $status ) {
			wp_send_json_success( array(
				'url'     => $base_url,
				'uid'     => $uid,
				'message' => __( 'Your listing is under review and will be published shortly.', '6arshid-social-community-main' ),
			) );
		}
		...
		wp_send_json_success( array(
			...
			'message' => 'draft' === $status
				? __( 'Draft saved.', '6arshid-social-community-main' )
				: __( 'Your listing is now live!', '6arshid-social-community-main' ),
		) );
	}
	...
	public function ajax_delete_listing(): void {
		...
		if ( ! $listing_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid listing.', '6arshid-social-community-main' ) ), 400 );
		}
		...
		if ( ! $listing ) {
			wp_send_json_error( array( 'message' => __( 'Listing not found.', '6arshid-social-community-main' ) ), 404 );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Listing deleted.', '6arshid-social-community-main' ) ) );
	}
	...
	public function ajax_report_listing(): void {
		...
		if ( in_array( $listing_id, array_map( 'intval', $reported ), true ) ) {
			wp_send_json_error( array( 'message' => __( 'You have already reported this listing.', '6arshid-social-community-main' ) ), 409 );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Thank you. Your report has been submitted.', '6arshid-social-community-main' ) ) );
	}
