...
class Marketplace_Listings {
	...
	public function ajax_upload_photo(): void {
		...
		set_transient( "arshid6social_mkt_draft_{$token}", $draft, DAY_IN_SECONDS );
	...
	public function ajax_save_listing(): void {
		...
		if ( $token ) {
			$draft_photos = get_transient( "arshid6social_mkt_draft_{$token}" ) ?: array();
			...
			delete_transient( "arshid6social_mkt_draft_{$token}" );
		}
