...
class Bookmarks {
	...
	public function add_page_definition( array $pages ): array {
		...
		$pages['saved_posts'] = array(
			'title'       => __( 'Saved Posts', '6arshid-social-community-main' ),
			'slug'        => 'saved-posts',
			'shortcode'   => '[arshid6social_bookmarks]',
			'option'      => 'arshid6social_page_saved_posts',
			'description' => __( 'Bookmarked posts and saved marketplace listings', '6arshid-social-community-main' ),
		);
		return $pages;
	}
			...
			if ( ! empty( $saved_ids ) ) {
				...
				foreach ( $listing_rows as $listing ) {
					...
					if ( ! empty( $listing->is_free ) ) {
						$price_formatted = __( 'Free', '6arshid-social-community-main' );
					} else {
	...
	public function shortcode( array $atts ): string {
		if ( ! is_user_logged_in() ) {
			return '<p>' . esc_html__( 'Please log in to view your bookmarks.', '6arshid-social-community-main' ) . '</p>';
		}
