...
class Advanced_Polls {
	...
	public function ajax_upload_option_image(): void {
		...
		if ( empty( $_FILES['image']['tmp_name'] ) ) {
			wp_send_json_error( array( 'message' => __( 'No image uploaded.', 'social-network-6' ) ), 400 );
		}
		...
		if ( (int) $file['size'] > $max_bytes ) {
			wp_send_json_error( array( 'message' => __( 'Image too large.', 'social-network-6' ) ), 413 );
		}
		...
		if ( ! in_array( $real_mime, $allowed, true ) ) {
			wp_send_json_error( array( 'message' => __( 'Only JPEG, PNG, and WebP images are allowed.', 'social-network-6' ) ), 415 );
		}

		// Re-encode image to strip EXIF/GPS.
		$image = $this->re_encode_image( $file['tmp_name'], $real_mime );
		if ( ! $image ) {
			wp_send_json_error( array( 'message' => __( 'Could not process image.', 'social-network-6' ) ), 500 );
		}
