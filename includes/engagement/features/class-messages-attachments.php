...
class Messages_Attachments {
	...
	private function do_upload(): void {
		...
		if ( ! $message_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid message ID.', 'social-network-6' ) ), 400 );
		}
		...
		if ( (int) $file['size'] > $max_bytes ) {
			wp_send_json_error( array( 'message' => __( 'File too large.', 'social-network-6' ) ), 413 );
		}
		...
		if ( ! in_array( $real_mime, $allowed_mimes, true ) ) {
			wp_send_json_error( array( 'message' => __( 'File type not allowed.', 'social-network-6' ) ), 415 );
		}
		...
		if ( ! isset( $moved['file'] ) || isset( $moved['error'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Upload failed.', 'social-network-6' ) ), 500 );
		}
