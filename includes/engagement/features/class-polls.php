...
class Polls {
	...
	public function vote( int $poll_id, array $option_ids, int $user_id ): array {
		...
		if ( ! $poll ) {
			return array( 'success' => false, 'message' => __( 'Poll not found.', '6arshid-social-community-main' ) );
		}

		if ( 'open' !== $poll->status ) {
			return array( 'success' => false, 'message' => __( 'This poll is closed.', '6arshid-social-community-main' ) );
		}

		if ( $poll->start_date && strtotime( $poll->start_date ) > time() ) {
			return array( 'success' => false, 'message' => __( 'This poll has not started yet.', '6arshid-social-community-main' ) );
		}

		$has_voted = $this->user_has_voted( $poll_id, $user_id );

		if ( $has_voted && ! $poll->allow_change_vote ) {
			return array( 'success' => false, 'message' => __( 'You have already voted.', '6arshid-social-community-main' ) );
		}
		...
		if ( empty( $option_ids ) ) {
			return array( 'success' => false, 'message' => __( 'Invalid option(s).', '6arshid-social-community-main' ) );
		}
		...
		return array( 'success' => true, 'message' => __( 'Vote recorded.', '6arshid-social-community-main' ), 'results' => $this->get_results( $poll_id, $user_id ) );
	}
	...
	public function ajax_create(): void {
		...
		if ( ! $activity_id || ! $question || count( $options ) < 2 ) {
			wp_send_json_error( array( 'message' => __( 'Question and at least 2 options required.', '6arshid-social-community-main' ) ), 400 );
		}
	...
	public function render_html( int $poll_id, int $viewer_id = 0 ): string {
		...
		$vote_btn = ( ! $has_voted && 'open' === $poll->status )
			? '<button type="button" class="arshid6social-btn arshid6social-btn--primary arshid6social-btn--sm arshid6social-poll-vote-btn" onclick="ARSHID6SOCIALVote(this)">' . esc_html__( 'Vote', '6arshid-social-community-main' ) . '</button>'
			: '';

		$meta_text = sprintf(
			/* translators: %d: vote count */
			esc_html( _n( '%d vote', '%d votes', $total, '6arshid-social-community-main' ) ),
			$total
		);
	...
	public function ajax_vote(): void {
		...
		if ( $count >= 60 ) {
			wp_send_json_error( array( 'message' => __( 'Too many votes. Please slow down.', '6arshid-social-community-main' ) ), 429 );
		}
	...
	public function ajax_suggest_option(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Your suggestion has been submitted for review.', '6arshid-social-community-main' ) ) );
	}
