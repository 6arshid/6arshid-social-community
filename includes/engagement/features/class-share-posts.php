...
class Share_Posts {
	...
	public function share( int $user_id, int $original_id, string $comment = '', string $target_type = 'profile', int $target_id = 0 ): int|false {
		...
		$content .= '<cite><a href="' . esc_url( home_url( '/members/' . ( $orig_author ? $orig_author->user_nicename : '' ) . '/' ) ) . '">'
			. esc_html( $orig_author ? $orig_author->display_name : __( 'Deleted user', 'social-network-6' ) )
			. '</a></cite>';
	...
	public function ajax_share(): void {
		...
		if ( $count >= $limit ) {
			wp_send_json_error( array( 'message' => __( 'Rate limit exceeded. Please wait before sharing again.', 'social-network-6' ) ), 429 );
		}
		...
		if ( ! $new_id ) {
			wp_send_json_error( array( 'message' => __( 'Could not share this post.', 'social-network-6' ) ), 400 );
		}
	...
	public function ajax_share_to_message(): void {
		...
		if ( ! $activity || 'private' === $activity->privacy ) {
			wp_send_json_error( array( 'message' => __( 'This post cannot be shared.', 'social-network-6' ) ), 403 );
		}

		$url     = \Arshid6Social\Components\Activity\Activity::get_permalink( (int) $activity->id );
		/* translators: %s: post URL */
		$content = sprintf( __( 'Shared a post: %s', 'social-network-6' ), esc_url( $url ) );

		$thread_id = $msg_comp->start_thread(
			array( $recipient_id ),
			__( 'Shared post', 'social-network-6' ),
			$content,
			get_current_user_id()
		);
