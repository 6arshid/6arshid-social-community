...
class Social_Embeds_Renderer {
	...
	private static function lazy_wrap(
		...
		$btn_label = $prov_name
			...
			? sprintf( esc_attr__( 'Load %s embed', 'social-network-6' ), $prov_name )
			: esc_attr__( 'Load embed', 'social-network-6' );

		$click_label = $prov_name
			/* translators: %s: embed provider name */
			? sprintf( esc_html__( 'Click to load %s', 'social-network-6' ), $prov_name )
			: esc_html__( 'Click to load', 'social-network-6' );
		...
		$meta = '<div class="arshid6social-embed-meta">'
			...
			. esc_html__( 'View original', 'social-network-6' )
			. '</a>'
			. '</div>';
