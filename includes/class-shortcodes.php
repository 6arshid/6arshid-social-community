...
class Shortcodes {
	...
	public function who_to_follow( $atts ): string {
		...
		foreach ( $result['members'] as $m ) {
			...
			$labels = array(
				...
				'pending_sent'     => __( 'Pending', 'social-network-6' ),
				'pending_received' => __( 'Accept', 'social-network-6' ),
				'friends'          => __( 'Friends', 'social-network-6' ),
			);
			$btn_label = $labels[ $status ] ?? __( 'Add Friend', 'social-network-6' );
	...
	public function activity( $atts ): string {
		...
		<div class="arshid6social-activity-block" data-scope="<?php echo esc_attr( $scope ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>">
			...
			<form class="arshid6social-activity-form arshid6social-card" data-nonce="<?php echo esc_attr( $nonce ); ?>">
				...
				<div class="sixarshidsc-price-row" style="display:none;align-items:center;gap:6px;margin:0 0 8px;flex-wrap:wrap;">
					<label class="sixarshidsc-price-label" for="sixarshidsc-ppv-price-input" style="font-size:.875rem;white-space:nowrap;">
						<?php esc_html_e( '💰 Price to unlock:', 'social-network-6' ); ?>
					</label>
					<input type="number" id="sixarshidsc-ppv-price-input" name="ppv_price"
						min="0.50" step="0.01" placeholder="<?php esc_attr_e( 'e.g. 10.00', 'social-network-6' ); ?>"
				...
				<div class="arshid6social-activity-form-footer">
					<select name="privacy" class="arshid6social-privacy-select">
						<option value="public"><?php esc_html_e( '🌐 Public', 'social-network-6' ); ?></option>
						<option value="friends"><?php esc_html_e( '👥 Friends', 'social-network-6' ); ?></option>
						<option value="private"><?php esc_html_e( '🔒 Only Me', 'social-network-6' ); ?></option>
						<?php if ( get_option( 'sixarshidsc_enabled' ) ) : ?>
						<option value="paid"><?php esc_html_e( '💰 Paid', 'social-network-6' ); ?></option>
						<?php endif; ?>
					</select>
			...
			<?php endif; ?>
			<?php if ( $show_feed ) : ?>
			<div class="arshid6social-activity-feed" aria-label="<?php esc_attr_e( 'Activity feed', 'social-network-6' ); ?>"></div>
			...
			return '<div class="arshid6social-notice arshid6social-notice-info">' .
				wp_kses(
					sprintf(
						...
						__( 'Please <a href="%s">log in</a> to view your messages.', 'social-network-6' ),
		...
		if ( ! is_user_logged_in() ) {
			return '<div class="arshid6social-notice arshid6social-notice-info">' .
				wp_kses(
					sprintf(
						/* translators: %s: login URL */
						__( 'Please <a href="%s">log in</a> to view your notifications.', 'social-network-6' ),
						esc_url( wp_login_url( get_permalink() ) )
					),
					array( 'a' => array( 'href' => array() ) )
				) .
	...
	public function login_form( $atts ): string {
		...
		ob_start();
		$this->splash_wrap_start( __( 'Sign in', 'social-network-6' ) );
		?>
		<?php
		wp_login_form( array(
			'redirect'       => $redirect,
			'label_username' => __( 'Username or Email', 'social-network-6' ),
			'label_password' => __( 'Password', 'social-network-6' ),
			'label_remember' => __( 'Remember Me', 'social-network-6' ),
			'label_log_in'   => __( 'Log In', 'social-network-6' ),
		...
		<p class="a6scsplash__forgot">
			<a href="<?php echo esc_url( $forgot_url ); ?>"><?php esc_html_e( 'Forgot password?', 'social-network-6' ); ?></a>
		</p>
		<?php if ( $register_url ) : ?>
			<div class="a6scsplash__register-row">
				<span><?php esc_html_e( "Don't have an account?", 'social-network-6' ); ?></span>
				<a class="a6scsplash__register-btn a6scsplash-glass" href="<?php echo esc_url( $register_url ); ?>">
					<?php esc_html_e( 'Create account', 'social-network-6' ); ?>
				</a>
			</div>
		<?php endif; ?>
		<?php
	...
	public function register_form( $atts ): string {
		...
		if ( ! get_option( 'arshid6social_allow_registration', true ) ) {
			return '<p class="arshid6social-notice arshid6social-notice-info">' . esc_html__( 'Registration is currently closed.', 'social-network-6' ) . '</p>';
		}
		...
		if ( isset( $_POST['arshid6social_register_nonce'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			if ( ! check_ajax_referer( 'arshid6social_register', 'arshid6social_register_nonce', false ) ) {
				$messages[] = array( 'type' => 'error', 'text' => __( 'Security check failed.', 'social-network-6' ) );
			} else {
				...
				$errors = new \WP_Error();
				if ( ! $username ) {
					$errors->add( 'empty_username', __( 'Please enter a username.', 'social-network-6' ) );
				} else {
					...
					if ( $restriction_errors->has_errors() ) {
						...
						$errors->add( 'username_exists', __( 'That username is already taken.', 'social-network-6' ) );
					}
				}
				if ( ! $email ) {
					$errors->add( 'empty_email', __( 'Please enter your email address.', 'social-network-6' ) );
				} elseif ( email_exists( $email ) ) {
					$errors->add( 'email_exists', __( 'That email is already registered.', 'social-network-6' ) );
				}
				if ( strlen( $password ) < 8 ) {
					$errors->add( 'weak_password', __( 'Password must be at least 8 characters.', 'social-network-6' ) );
				}
		...
		ob_start();
		$this->splash_wrap_start( __( 'Create your account', 'social-network-6' ) );
		?>
		...
		<form class="a6scsplash__inner-form" method="post" novalidate>
			...
			<div class="a6scsplash__field">
				<label for="arshid6social-reg-username"><?php esc_html_e( 'Username', 'social-network-6' ); ?> <span aria-hidden="true">*</span></label>
			...
			<div class="a6scsplash__field">
				<label for="arshid6social-reg-email"><?php esc_html_e( 'Email address', 'social-network-6' ); ?> <span aria-hidden="true">*</span></label>
			...
			<div class="a6scsplash__field">
				<label for="arshid6social-reg-password"><?php esc_html_e( 'Password', 'social-network-6' ); ?> <span aria-hidden="true">*</span></label>
			...
			<div style="position:absolute;left:-9999px;" aria-hidden="true">
				<label for="arshid6social-hp"><?php esc_html_e( 'Leave this field empty', 'social-network-6' ); ?></label>
				<input type="text" id="arshid6social-hp" name="hp_field" tabindex="-1" autocomplete="off" />
			</div>

			<button type="submit" class="a6scsplash__submit-btn"><?php esc_html_e( 'Create Account', 'social-network-6' ); ?></button>

			<div class="a6scsplash__register-row" style="margin-top:1.25rem;">
				<span><?php esc_html_e( 'Already have an account?', 'social-network-6' ); ?></span>
				<a class="a6scsplash__register-btn a6scsplash-glass" href="<?php echo esc_url( $login_url ); ?>">
					<?php esc_html_e( 'Sign in', 'social-network-6' ); ?>
				</a>
			</div>
		</form>
		<script>
		(function(){
			...
			var timer    = null;
			var nonce    = <?php echo wp_json_encode( wp_create_nonce( 'arshid6social_check_username' ) ); ?>;
			var ajaxUrl  = <?php echo wp_json_encode( admin_url( 'admin-ajax.php' ) ); ?>;

			if ( ! input || ! feedback ) return;

			var minLen = <?php echo (int) get_option( 'arshid6social_username_min_length', 4 ); ?>;
			...
			input.addEventListener('input', function(){
				...
				feedback.textContent   = '<?php esc_html_e( 'Checking…', 'social-network-6' ); ?>';

				timer = setTimeout(function(){
					var data = new FormData();
					data.append('action',   'arshid6social_check_username');
					data.append('nonce',    nonce);
					data.append('username', val);
		...
		if ( ! $allow_guests && ! is_user_logged_in() ) {
			return '<div class="arshid6social-notice arshid6social-notice-info">' .
				wp_kses(
					sprintf(
						/* translators: %s: login URL */
						__( 'Please <a href="%s">log in</a> to browse the Marketplace.', 'social-network-6' ),
						esc_url( wp_login_url( get_permalink() ) )
					),
					array( 'a' => array( 'href' => array() ) )
				) . '</div>';
		...
		if ( ! is_user_logged_in() ) {
			return '<div class="arshid6social-notice arshid6social-notice-info">' .
				wp_kses(
					sprintf(
						/* translators: %s: login URL */
						__( 'Please <a href="%s">log in</a> to post a listing.', 'social-network-6' ),
						esc_url( wp_login_url( get_permalink() ) )
					),
					array( 'a' => array( 'href' => array() ) )
				) . '</div>';
		...
		if ( get_option( 'arshid6social_marketplace_require_verified', false ) ) {
			...
			if ( $verification && ! $verification->is_verified( get_current_user_id() ) ) {
				return '<div class="arshid6social-notice arshid6social-notice-warning">' .
					esc_html__( 'Only verified users can post listings.', 'social-network-6' ) .
					'</div>';
			}
		}
	...
	private function listing_detail(): string {
		...
		if ( ! $uid ) {
			return '<p class="arshid6social-notice">' . esc_html__( 'Listing not found.', 'social-network-6' ) . '</p>';
		}
		...
		if ( ! in_array( $listing->status, array( 'active', 'sold' ), true ) && ! $is_owner && ! $is_admin ) {
			return '<p class="arshid6social-notice">' . esc_html__( 'This listing is not available.', 'social-network-6' ) . '</p>';
		}
	...
	public function my_listings( $atts ): string {
		...
		return '<p class="arshid6social-notice">' . esc_html__( 'My listings coming soon.', 'social-network-6' ) . '</p>';
	}
	...
	public function saved_listings( $atts ): string {
		...
		return '<p class="arshid6social-notice">' . esc_html__( 'Saved listings coming soon.', 'social-network-6' ) . '</p>';
	}
	...
	public function seller_listings( $atts ): string {
		...
		return '<p class="arshid6social-notice">' . esc_html__( 'Seller listings coming soon.', 'social-network-6' ) . '</p>';
	}
	...
	public function forgot_password(): string {
		...
		if ( isset( $_POST['arshid6social_forgot_nonce'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			if ( ! check_ajax_referer( 'arshid6social_forgot_password', 'arshid6social_forgot_nonce', false ) ) {
				$error = __( 'Security check failed.', 'social-network-6' );
			} else {
				$user_login = sanitize_text_field( wp_unslash( $_POST['user_login'] ?? '' ) );

				if ( empty( $user_login ) ) {
					$error = __( 'Please enter your email or username.', 'social-network-6' );
				} else {
					...
					if ( ! $user || ! $user->exists() ) {
						$message = __( 'If an account with that information exists, a reset link has been sent.', 'social-network-6' );
					} else {
						$key = get_password_reset_key( $user );

						if ( is_wp_error( $key ) ) {
							$error = __( 'Could not generate reset link. Please try again.', 'social-network-6' );
						} else {
							$url = add_query_arg(
								array(
		...
		$this->splash_wrap_start(
			__( 'Forgot password?', 'social-network-6' ),
			__( 'Enter your email or username and we\'ll send you a reset link.', 'social-network-6' )
		);
		?>
		<?php if ( $message ) : ?>
		...
		<form id="arshid6social-forgot-form" class="a6scsplash__inner-form" method="post">
			...
			<div class="a6scsplash__field">
				<label for="arshid6social-forgot-login"><?php esc_html_e( 'Email or Username', 'social-network-6' ); ?></label>
			...
			<button type="submit" class="a6scsplash__submit-btn"><?php esc_html_e( 'Send Reset Link', 'social-network-6' ); ?></button>
		</form>
		<?php endif; ?>

		<p class="a6scsplash__forgot" style="text-align:center;margin-top:1rem;">
			<a href="<?php echo esc_url( $login_url ); ?>"><?php esc_html_e( '← Back to Sign in', 'social-network-6' ); ?></a>
		</p>
		<?php
		$this->splash_wrap_end();
		return ob_get_clean();
	...
	public function reset_password(): string {
		...
		if ( $key && $login ) {
			...
			if ( is_wp_error( $user ) ) {
				$error = __( 'This password reset link is invalid or has expired. Please request a new one.', 'social-network-6' );
				$user  = null;
			}
		} else {
			$error = __( 'Invalid password reset link.', 'social-network-6' );
		}
		...
		if ( $user && isset( $_POST['arshid6social_reset_nonce'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			if ( ! check_ajax_referer( 'arshid6social_reset_password', 'arshid6social_reset_nonce', false ) ) {
				...
				if ( strlen( $pass1 ) < 8 ) {
					$error = __( 'Password must be at least 8 characters.', 'social-network-6' );
				} elseif ( $pass1 !== $pass2 ) {
					$error = __( 'Passwords do not match.', 'social-network-6' );
				} else {
					reset_password( $user, $pass1 );
					$message = __( 'Your password has been reset. You can now log in.', 'social-network-6' );
					$user    = null;
				}
			}
		}
		...
		ob_start();
		$this->splash_wrap_start( __( 'Set new password', 'social-network-6' ) );
		?>
		<?php if ( $message ) : ?>
			<div class="arshid6social-notice arshid6social-notice-success"><?php echo esc_html( $message ); ?></div>
			<p class="a6scsplash__forgot" style="text-align:center;margin-top:1rem;">
				<a href="<?php echo esc_url( $login_url ); ?>"><?php esc_html_e( 'Sign in →', 'social-network-6' ); ?></a>
			</p>
		<?php elseif ( $error && ! $user ) : ?>
			<div class="arshid6social-notice arshid6social-notice-error"><?php echo esc_html( $error ); ?></div>
			<?php if ( $forgot_url ) : ?>
				<p class="a6scsplash__forgot" style="text-align:center;margin-top:1rem;">
					<a href="<?php echo esc_url( $forgot_url ); ?>"><?php esc_html_e( 'Request a new reset link', 'social-network-6' ); ?></a>
				</p>
			<?php endif; ?>
		<?php elseif ( $user ) : ?>
			<?php if ( $error ) : ?>
			...
			<form id="arshid6social-reset-form" class="a6scsplash__inner-form" method="post">
				...
				<div class="a6scsplash__field">
					<label for="arshid6social-reset-pass1"><?php esc_html_e( 'New Password', 'social-network-6' ); ?></label>
					<input type="password" id="arshid6social-reset-pass1" name="password1"
						autocomplete="new-password" minlength="8">
				</div>

				<div class="a6scsplash__field">
					<label for="arshid6social-reset-pass2"><?php esc_html_e( 'Confirm Password', 'social-network-6' ); ?></label>
					<input type="password" id="arshid6social-reset-pass2" name="password2"
						autocomplete="new-password" minlength="8">
				</div>

				<button type="submit" class="a6scsplash__submit-btn"><?php esc_html_e( 'Set New Password', 'social-network-6' ); ?></button>
			</form>
		<?php endif; ?>
		<?php
		$this->splash_wrap_end();
		...
		<div class="a6scsplash">
			...
			<div class="a6scsplash__hero">
				...
				<h1 class="a6scsplash__headline animate-fade-rise">
					<?php esc_html_e( 'Where ', 'social-network-6' ); ?>
					<em><?php esc_html_e( 'your community', 'social-network-6' ); ?></em>
					<?php esc_html_e( ' comes alive.', 'social-network-6' ); ?>
				</h1>

				<p class="a6scsplash__sub animate-fade-rise-delay">
					<?php esc_html_e( 'Connect, share, and grow with people who matter.', 'social-network-6' ); ?>
				</p>
