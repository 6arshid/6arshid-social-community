...
class Media_Handler {
	...
	public static function handle( array $file, string $context, int $user_id ): array|\WP_Error {
		if ( ! isset( self::CONTEXTS[ $context ] ) ) {
			return new \WP_Error( 'invalid_context', __( 'Invalid upload context.', '6arshid-social-community-main' ) );
		}
		...
		if ( $file['size'] > $max_bytes ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'File exceeds the maximum size of %s MB.', '6arshid-social-community-main' ),
		...
		if ( ! in_array( $real_mime, $cfg['allowed_mime'], true ) ) {
			return new \WP_Error( 'invalid_mime', __( 'File type not allowed.', '6arshid-social-community-main' ) );
		}
		...
		if ( ! is_uploaded_file( $file['tmp_name'] ) ) {
			return new \WP_Error( 'move_failed', __( 'Failed to save uploaded file.', '6arshid-social-community-main' ) );
		}
