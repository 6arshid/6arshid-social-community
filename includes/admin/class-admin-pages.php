	...
	public function render(): void {
		...
		document.querySelectorAll( '.arshid6social-recreate-btn' ).forEach( btn => {
			btn.addEventListener( 'click', async () => {
				if ( ! confirm( '<?php echo esc_js( __( 'Re-create this page?', '6arshid social community' ) ); ?>' ) ) return;
				btn.disabled = true;
		...
		document.querySelectorAll( '.arshid6social-assign-save' ).forEach( btn => {
			btn.addEventListener( 'click', async () => {
				...
				try {
					...
					const data = await res.json();
					if ( data.success ) {
						msgEl.textContent    = '✓ <?php echo esc_js( __( 'Saved', '6arshid social community' ) ); ?>';
						msgEl.style.color    = '#16a34a';
						msgEl.style.display  = 'inline';
						setTimeout( () => { location.reload(); }, 900 );
					} else {
						msgEl.textContent   = data.data?.message || '<?php echo esc_js( __( 'Error.', '6arshid social community' ) ); ?>';
						msgEl.style.color   = '#dc2626';
						msgEl.style.display = 'inline';
					}
				} catch {
					msgEl.textContent   = '<?php echo esc_js( __( 'Error.', '6arshid social community' ) ); ?>';
					msgEl.style.color   = '#dc2626';
					msgEl.style.display = 'inline';
				} finally {
					btn.disabled = false;
