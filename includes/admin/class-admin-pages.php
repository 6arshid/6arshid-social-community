...
class Admin_Pages {
	...
	public function render(): void {
		...
		<div class="wrap arshid6social-admin-pages">
			<h1><?php esc_html_e( 'Social Network — Pages & Shortcodes', 'social-network-6' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'These pages were created automatically when the plugin was activated. Each page uses a shortcode to display the social network content. You can copy the shortcode and paste it into any page or widget.', 'social-network-6' ); ?>
			</p>
		...
		<style>
			.arshid6social-pages-table { width:100%; border-collapse:collapse; margin-block-start:1.5rem; }
			.arshid6social-pages-table th,
			.arshid6social-pages-table td { padding:.75rem 1rem; border-bottom:1px solid #e2e8f0; text-align:left; }
			.arshid6social-pages-table th { background:#f8fafc; font-weight:600; font-size:.8125rem; text-transform:uppercase; letter-spacing:.04em; color:#475569; }
			.arshid6social-pages-table tr:last-child td { border-bottom:none; }
			.arshid6social-shortcode-box { display:flex; align-items:center; gap:.5rem; }
			.arshid6social-shortcode-code { font-family:monospace; background:#f1f5f9; border:1px solid #cbd5e1; border-radius:4px; padding:.25rem .625rem; font-size:.875rem; color:#1e40af; }
			.arshid6social-copy-btn { cursor:pointer; background:#e0e7ff; border:none; border-radius:4px; padding:.25rem .625rem; font-size:.8125rem; color:#3730a3; }
			...
			.arshid6social-recreate-btn { background:#fef9c3; border:1px solid #fde047; border-radius:4px; padding:.25rem .75rem; cursor:pointer; font-size:.8125rem; color:#713f12; }
			.arshid6social-sc-ref { margin-block-start:2rem; }
			.arshid6social-sc-ref table { width:100%; border-collapse:collapse; }
			.arshid6social-sc-ref th, .arshid6social-sc-ref td { padding:.625rem 1rem; border-bottom:1px solid #e2e8f0; text-align:left; font-size:.875rem; }
			.arshid6social-sc-ref th { background:#f8fafc; font-weight:600; color:#475569; }
			.arshid6social-sc-ref code { font-family:monospace; background:#f1f5f9; padding:.2rem .5rem; border-radius:3px; color:#1e40af; }
			.arshid6social-assign-select { font-size:.875rem; border-radius:4px; border:1px solid #cbd5e1; padding:.3rem .5rem; }
			.arshid6social-assign-view { font-size:1rem; text-decoration:none; opacity:.7; }
			.arshid6social-assign-view:hover { opacity:1; }
		</style>

		<script>
		document.querySelectorAll( '.arshid6social-copy-btn' ).forEach( btn => {
		...
		document.querySelectorAll( '.arshid6social-recreate-btn' ).forEach( btn => {
			btn.addEventListener( 'click', async () => {
				if ( ! confirm( '<?php echo esc_js( __( 'Re-create this page?', 'social-network-6' ) ); ?>' ) ) return;
				btn.disabled = true;
				const body = new FormData();
				body.append( 'action', 'arshid6social_recreate_page' );
				body.append( 'nonce', '<?php echo esc_js( wp_create_nonce( 'arshid6social_recreate_page' ) ); ?>' );
		...
		document.querySelectorAll( '.arshid6social-assign-save' ).forEach( btn => {
			btn.addEventListener( 'click', async () => {
				const wrap    = btn.closest( '.arshid6social-assign-page-wrap' );
				const select  = wrap.querySelector( '.arshid6social-assign-select' );
				const msgEl   = wrap.querySelector( '.arshid6social-assign-msg' );
				...
				body.append( 'nonce', '<?php echo esc_js( wp_create_nonce( 'arshid6social_assign_page' ) ); ?>' );
				...
				try {
					...
					const data = await res.json();
					if ( data.success ) {
						msgEl.textContent    = '✓ <?php echo esc_js( __( 'Saved', 'social-network-6' ) ); ?>';
						msgEl.style.color    = '#16a34a';
						msgEl.style.display  = 'inline';
						setTimeout( () => { location.reload(); }, 900 );
					} else {
						msgEl.textContent   = data.data?.message || '<?php echo esc_js( __( 'Error.', 'social-network-6' ) ); ?>';
						msgEl.style.color   = '#dc2626';
						msgEl.style.display = 'inline';
					}
				} catch {
					msgEl.textContent   = '<?php echo esc_js( __( 'Error.', 'social-network-6' ) ); ?>';
					msgEl.style.color   = '#dc2626';
					msgEl.style.display = 'inline';
				} finally {
					btn.disabled = false;
	...
	private function render_pages_table( array $pages ): void {
		...
		<table class="arshid6social-pages-table widefat">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Page', 'social-network-6' ); ?></th>
					<th><?php esc_html_e( 'Status', 'social-network-6' ); ?></th>
					<th><?php esc_html_e( 'Assigned Page', 'social-network-6' ); ?></th>
					<th><?php esc_html_e( 'Shortcode', 'social-network-6' ); ?></th>
			...
			<tbody>
				<?php foreach ( $pages as $key => $page ) : ?>
					...
					<tr>
						...
						<td>
							<?php if ( $is_ok ) : ?>
								<span class="arshid6social-status-ok">&#10003; <?php esc_html_e( 'Active', 'social-network-6' ); ?></span>
							<?php else : ?>
								<span class="arshid6social-status-missing">&#10007; <?php esc_html_e( 'Missing', 'social-network-6' ); ?></span>
							<?php endif; ?>
						</td>
						<td>
							<div class="arshid6social-assign-page-wrap" style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;">
								<select class="arshid6social-assign-select" data-page-key="<?php echo esc_attr( $key ); ?>" data-option="<?php echo esc_attr( $page['option'] ); ?>" style="max-width:260px;">
									<option value="0"><?php esc_html_e( '— Not assigned —', 'social-network-6' ); ?></option>
								...
								<button type="button" class="button arshid6social-assign-save" data-page-key="<?php echo esc_attr( $key ); ?>">
									<?php esc_html_e( 'Save', 'social-network-6' ); ?>
								</button>
								<?php if ( $is_ok ) : ?>
									<a href="<?php echo esc_url( $page_url ); ?>" target="_blank" class="arshid6social-assign-view" title="<?php esc_attr_e( 'View page', 'social-network-6' ); ?>">&#128279;</a>
								<?php endif; ?>
						...
						<td>
							<div class="arshid6social-shortcode-box">
								...
								<button type="button" class="arshid6social-copy-btn"><?php esc_html_e( 'Copy', 'social-network-6' ); ?></button>
							</div>
						</td>
						<td>
							<?php if ( ! $is_ok ) : ?>
								<button type="button" class="arshid6social-recreate-btn" data-page-key="<?php echo esc_attr( $key ); ?>">
									<?php esc_html_e( 'Re-create Page', 'social-network-6' ); ?>
								</button>
							<?php else : ?>
								—
							<?php endif; ?>
	...
	private function render_shortcode_reference(): void {
		$shortcodes = array(
			array(
				...
				'desc'    => __( 'Displays the member directory with search bar and pagination.', 'social-network-6' ),
			...
			array(
				...
				'desc'    => __( 'Displays an activity feed with infinite scroll. Logged-in users see a post composer.', 'social-network-6' ),
			...
			array(
				...
				'desc'    => __( 'Displays the group directory. Logged-in users see a "Create Group" button.', 'social-network-6' ),
				'example' => '[arshid6social_groups per_page="12" status="public"]',
			),
			array(
				'code'    => '[arshid6social_messages]',
				'attrs'   => '—',
				'desc'    => __( 'Displays the private messages inbox. Redirects guests to login.', 'social-network-6' ),
				'example' => '[arshid6social_messages]',
			),
			array(
				'code'    => '[arshid6social_notifications]',
				'attrs'   => '—',
				'desc'    => __( 'Displays the current user\'s notifications feed with mark-all-read and customizable preferences. Redirects guests to login.', 'social-network-6' ),
				'example' => '[arshid6social_notifications]',
			),
			array(
				'code'    => '[arshid6social_profile]',
				'attrs'   => 'id="0" slug=""',
				'desc'    => __( 'Displays a user profile. Uses the current logged-in user if no id or slug is given.', 'social-network-6' ),
				'example' => '[arshid6social_profile id="5"]',
			),
			array(
				'code'    => '[arshid6social_login_form]',
				'attrs'   => 'redirect=""',
				'desc'    => __( 'Displays a styled login form. Hidden when the user is already logged in.', 'social-network-6' ),
				'example' => '[arshid6social_login_form redirect="/members/"]',
			),
			array(
				'code'    => '[arshid6social_register_form]',
				'attrs'   => '—',
				'desc'    => __( 'Displays a registration form. Respects the "Allow Registration" setting.', 'social-network-6' ),
				'example' => '[arshid6social_register_form]',
			),
			array(
				'code'    => '[arshid6social_forgot_password]',
				'attrs'   => '—',
				'desc'    => __( 'Custom forgot-password form. Sends a reset email linking to the Reset Password page — no wp-login.php.', 'social-network-6' ),
				'example' => '[arshid6social_forgot_password]',
			),
			array(
				'code'    => '[arshid6social_reset_password]',
				'attrs'   => '—',
				'desc'    => __( 'Custom reset-password form. Reads the key and login from the email link and sets a new password — no wp-login.php.', 'social-network-6' ),
				'example' => '[arshid6social_reset_password]',
			),
			array(
				'code'    => '[arshid6social_bookmarks]',
				'attrs'   => 'per_page="20"',
				'desc'    => __( 'Displays the current user\'s saved posts and bookmarked marketplace listings.', 'social-network-6' ),
				'example' => '[arshid6social_bookmarks per_page="20"]',
			),
		);
		?>
		<div class="arshid6social-sc-ref">
			<h2><?php esc_html_e( 'Shortcode Reference', 'social-network-6' ); ?></h2>
			<p class="description"><?php esc_html_e( 'All shortcodes can be placed in any page, post, or widget. Gutenberg users can also use the dedicated blocks (search for "Social Network" in the block inserter).', 'social-network-6' ); ?></p>

			<table class="widefat">
				<thead>
					<tr>
						<th style="width:180px;"><?php esc_html_e( 'Shortcode', 'social-network-6' ); ?></th>
						<th><?php esc_html_e( 'Available Attributes', 'social-network-6' ); ?></th>
						<th><?php esc_html_e( 'Description', 'social-network-6' ); ?></th>
						<th><?php esc_html_e( 'Example', 'social-network-6' ); ?></th>
					</tr>
				</thead>
				<tbody>
	...
	public function ajax_recreate_page(): void {
		...
		if ( ! isset( $pages[ $page_key ] ) ) {
			wp_send_json_error( array( 'message' => __( 'Unknown page key.', 'social-network-6' ) ) );
		}
	...
	public function ajax_assign_page(): void {
		...
		if ( $page_id && ! get_post( $page_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Page not found.', 'social-network-6' ) ) );
		}
	...
	private function get_page_definitions(): array {
		$pages = array(
			'members'  => array(
				...
				'description' => __( 'Member directory with search and filters', 'social-network-6' ),
			),
			'activity' => array(
				...
				'description' => __( 'Site-wide activity feed', 'social-network-6' ),
			),
			'groups'   => array(
				...
				'description' => __( 'Group directory with join buttons', 'social-network-6' ),
			),
			'messages' => array(
				...
				'description' => __( 'Private messaging inbox', 'social-network-6' ),
			),
			'notifications' => array(
				...
				'description' => __( 'Personal notifications feed with customize options', 'social-network-6' ),
			),
			'register' => array(
				...
				'description' => __( 'Member registration form', 'social-network-6' ),
			),
			'login' => array(
				...
				'description' => __( 'Member login form', 'social-network-6' ),
			),
			'forgot-password' => array(
				...
				'description' => __( 'Password reset request form (no wp-login.php)', 'social-network-6' ),
			),
			'reset-password' => array(
				...
				'description' => __( 'Set new password via email link (no wp-login.php)', 'social-network-6' ),
			),
		);
