...
final class Admin_Members {
	...
	public function get_columns(): array {
		return array(
			'member'     => __( 'Member', 'social-network-6' ),
			'email'      => __( 'Email', 'social-network-6' ),
			'registered' => __( 'Registered', 'social-network-6' ),
	...
	public function render(): void {
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Members', 'social-network-6' ); ?></h1>
			...
			<form method="get">
				...
				<p class="search-box">
					<label class="screen-reader-text" for="arshid6social-member-search"><?php esc_html_e( 'Search members', 'social-network-6' ); ?></label>
					<input type="search" id="arshid6social-member-search" name="s"
						value="<?php echo esc_attr( $search ); ?>"
						placeholder="<?php esc_attr_e( 'Search members…', 'social-network-6' ); ?>" />
					<?php submit_button( __( 'Search Members', 'social-network-6' ), 'secondary', '', false ); ?>
				</p>
			</form>
					...
					<span class="displaying-num">
						...
						echo esc_html(
							sprintf(
								...
								_n( '%s member', '%s members', $total_users, 'social-network-6' ),
			...
			<table class="wp-list-table widefat fixed striped">
				<thead>
					<tr>
						<th class="manage-column column-member column-primary"><?php esc_html_e( 'Member', 'social-network-6' ); ?></th>
						<th class="manage-column column-email<?php echo esc_attr( $col_class( 'email' ) ); ?>"><?php esc_html_e( 'Email', 'social-network-6' ); ?></th>
						<th class="manage-column column-registered<?php echo esc_attr( $col_class( 'registered' ) ); ?>"><?php esc_html_e( 'Registered', 'social-network-6' ); ?></th>
				...
				<tbody>
						...
						<tr>
							<td class="member column-member column-primary has-row-actions">
								...
								<div class="row-actions">
									<span>
										<a href="<?php echo esc_url( get_edit_user_link( $user->ID ) ); ?>"><?php esc_html_e( 'Edit', 'social-network-6' ); ?></a>
									</span>
								</div>
							</td>
							...
							<td class="status column-status<?php echo esc_attr( $col_class( 'status' ) ); ?>">
								<?php if ( $suspended ) : ?>
									...
									<?php elseif ( 'auto_threshold' === $susp_reason ) : ?>
										<br><small style="color:#888;"><?php esc_html_e( 'Auto (report threshold)', 'social-network-6' ); ?></small>
									<?php endif; ?>
								<?php else : ?>
							...
							<td class="actions column-actions<?php echo esc_attr( $col_class( 'actions' ) ); ?>">
								<button class="button button-small arshid6social-admin-suspend-btn"
									...
									<?php echo $suspended ? esc_html__( 'Unsuspend', 'social-network-6' ) : esc_html__( 'Suspend', 'social-network-6' ); ?>
								</button>
					...
					<?php if ( empty( $users ) ) : ?>
						<tr><td colspan="5"><?php esc_html_e( 'No members found.', 'social-network-6' ); ?></td></tr>
					<?php endif; ?>
				</tbody>
		...
		<div id="arshid6social-suspend-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:100000;align-items:center;justify-content:center;">
			<div style="background:#fff;border-radius:6px;padding:24px;max-width:480px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,.2);">
				<h2 style="margin-top:0;" id="arshid6social-sm-title"><?php esc_html_e( 'Suspend User', 'social-network-6' ); ?></h2>
				...
				<input type="text" id="arshid6social-sm-custom" placeholder="<?php esc_attr_e( 'Custom reason…', 'social-network-6' ); ?>"
					style="width:100%;display:none;margin-bottom:16px;" />
				<div style="display:flex;gap:8px;justify-content:flex-end;">
					<button class="button" id="arshid6social-sm-cancel"><?php esc_html_e( 'Cancel', 'social-network-6' ); ?></button>
					<button class="button button-primary" id="arshid6social-sm-confirm"><?php esc_html_e( 'Confirm', 'social-network-6' ); ?></button>
				</div>
	...
	public function ajax_delete_member_data(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Member data deleted.', 'social-network-6' ) ) );
	}
}
