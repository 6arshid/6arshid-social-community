...
class Admin_Verification {
	...
	public function setup_screen_options(): void {
		add_screen_option(
			'per_page',
			array(
				'label'   => __( 'Requests per page', 'social-network-6' ),
				'default' => 20,
	...
	public function get_columns(): array {
		return array(
			'user'      => __( 'User', 'social-network-6' ),
			'type'      => __( 'Type', 'social-network-6' ),
			'full_name' => __( 'Full Name', 'social-network-6' ),
			'category'  => __( 'Category', 'social-network-6' ),
			'links'     => __( 'Links', 'social-network-6' ),
			'doc'       => __( 'Doc', 'social-network-6' ),
			'submitted' => __( 'Submitted', 'social-network-6' ),
	...
	public function render(): void {
		...
		if ( isset( $_GET['arshid6social_msg'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$messages = array(
				'approved'  => __( 'Request approved and badge granted.', 'social-network-6' ),
				'rejected'  => __( 'Request rejected.', 'social-network-6' ),
				'more_info' => __( 'More info requested from user.', 'social-network-6' ),
				'revoked'   => __( 'Verification badge revoked.', 'social-network-6' ),
			);
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Verification Queue', 'social-network-6' ); ?></h1>
			...
			<ul class="subsubsub">
				<?php
				$tab_labels = array(
					'pending'  => __( 'Pending', 'social-network-6' ),
					'approved' => __( 'Approved', 'social-network-6' ),
					'rejected' => __( 'Rejected', 'social-network-6' ),
					'revoked'  => __( 'Revoked', 'social-network-6' ),
				);
				$tab_list = array();
			...
			<table class="wp-list-table widefat fixed striped">
				<thead>
					<tr>
						<th class="manage-column column-user column-primary"><?php esc_html_e( 'User', 'social-network-6' ); ?></th>
						<th class="manage-column column-type<?php echo esc_attr( $col_class( 'type' ) ); ?>"><?php esc_html_e( 'Type', 'social-network-6' ); ?></th>
						<th class="manage-column column-full_name<?php echo esc_attr( $col_class( 'full_name' ) ); ?>"><?php esc_html_e( 'Full Name', 'social-network-6' ); ?></th>
						<th class="manage-column column-category<?php echo esc_attr( $col_class( 'category' ) ); ?>"><?php esc_html_e( 'Category', 'social-network-6' ); ?></th>
						<th class="manage-column column-links<?php echo esc_attr( $col_class( 'links' ) ); ?>"><?php esc_html_e( 'Links', 'social-network-6' ); ?></th>
						<th class="manage-column column-doc<?php echo esc_attr( $col_class( 'doc' ) ); ?>"><?php esc_html_e( 'Doc', 'social-network-6' ); ?></th>
						<th class="manage-column column-submitted<?php echo esc_attr( $col_class( 'submitted' ) ); ?>"><?php esc_html_e( 'Submitted', 'social-network-6' ); ?></th>
						<th class="manage-column column-actions<?php echo esc_attr( $col_class( 'actions' ) ); ?>"><?php esc_html_e( 'Actions', 'social-network-6' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php if ( empty( $rows ) ) : ?>
						<tr><td colspan="8"><?php esc_html_e( 'No requests found.', 'social-network-6' ); ?></td></tr>
					<?php else : ?>
									...
									<?php if ( ! empty( $doc_paths ) ) : ?>
										<?php foreach ( $doc_paths as $idx => $path ) : ?>
											<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=arshid6social_serve_verification_doc&request_id=' . $row->id . '&idx=' . $idx . '&nonce=' . wp_create_nonce( 'arshid6social_ajax_nonce' ) ) ); ?>" target="_blank">
												...
												echo esc_html( sprintf( __( 'Doc %d', 'social-network-6' ), $idx + 1 ) ); ?>
											</a><br>
										<?php endforeach; ?>
									<?php else : ?>
										—
								...
								<td class="actions column-actions<?php echo esc_attr( $col_class( 'actions' ) ); ?>">
									<?php if ( 'pending' === $row->status ) : ?>
										<a href="<?php echo esc_url( wp_nonce_url(
											...
											<?php esc_html_e( 'Approve', 'social-network-6' ); ?>
										</a>
										<a href="<?php echo esc_url( wp_nonce_url(
											admin_url( 'admin.php?page=arshid6social-verification&arshid6social_verify_action=reject&request_id=' . $row->id ),
											'arshid6social_verify_action_' . $row->id
										) ); ?>" class="button button-small"
											onclick="return confirm('<?php esc_attr_e( 'Reject this request?', 'social-network-6' ); ?>')">
											<?php esc_html_e( 'Reject', 'social-network-6' ); ?>
										</a>
									<?php elseif ( 'approved' === $row->status ) : ?>
										...
										<a href="<?php echo esc_url( wp_nonce_url(
											...
											onclick="return confirm('<?php esc_attr_e( 'Revoke this verification badge?', 'social-network-6' ); ?>')">
											<?php esc_html_e( 'Revoke Badge', 'social-network-6' ); ?>
										</a>
