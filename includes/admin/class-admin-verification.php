...
class Admin_Verification {
	...
	public function setup_screen_options(): void {
		add_screen_option(
			'per_page',
			array(
				'label'   => __( 'Requests per page', '6arshid-social-community-main' ),
				'default' => 20,
	...
	public function get_columns(): array {
		return array(
			'user'      => __( 'User', '6arshid-social-community-main' ),
			'type'      => __( 'Type', '6arshid-social-community-main' ),
			'full_name' => __( 'Full Name', '6arshid-social-community-main' ),
			'category'  => __( 'Category', '6arshid-social-community-main' ),
			'links'     => __( 'Links', '6arshid-social-community-main' ),
			'doc'       => __( 'Doc', '6arshid-social-community-main' ),
			'submitted' => __( 'Submitted', '6arshid-social-community-main' ),
	...
	public function render(): void {
		...
		if ( isset( $_GET['arshid6social_msg'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$messages = array(
				'approved'  => __( 'Request approved and badge granted.', '6arshid-social-community-main' ),
				'rejected'  => __( 'Request rejected.', '6arshid-social-community-main' ),
				'more_info' => __( 'More info requested from user.', '6arshid-social-community-main' ),
				'revoked'   => __( 'Verification badge revoked.', '6arshid-social-community-main' ),
			);
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Verification Queue', '6arshid-social-community-main' ); ?></h1>
			...
			<ul class="subsubsub">
				<?php
				$tab_labels = array(
					'pending'  => __( 'Pending', '6arshid-social-community-main' ),
					'approved' => __( 'Approved', '6arshid-social-community-main' ),
					'rejected' => __( 'Rejected', '6arshid-social-community-main' ),
					'revoked'  => __( 'Revoked', '6arshid-social-community-main' ),
				);
				$tab_list = array();
			...
			<table class="wp-list-table widefat fixed striped">
				<thead>
					<tr>
						<th class="manage-column column-user column-primary"><?php esc_html_e( 'User', '6arshid-social-community-main' ); ?></th>
						<th class="manage-column column-type<?php echo esc_attr( $col_class( 'type' ) ); ?>"><?php esc_html_e( 'Type', '6arshid-social-community-main' ); ?></th>
						<th class="manage-column column-full_name<?php echo esc_attr( $col_class( 'full_name' ) ); ?>"><?php esc_html_e( 'Full Name', '6arshid-social-community-main' ); ?></th>
						...
						<th class="manage-column column-links<?php echo esc_attr( $col_class( 'links' ) ); ?>"><?php esc_html_e( 'Links', '6arshid-social-community-main' ); ?></th>
						<th class="manage-column column-doc<?php echo esc_attr( $col_class( 'doc' ) ); ?>"><?php esc_html_e( 'Doc', '6arshid-social-community-main' ); ?></th>
						<th class="manage-column column-submitted<?php echo esc_attr( $col_class( 'submitted' ) ); ?>"><?php esc_html_e( 'Submitted', '6arshid-social-community-main' ); ?></th>
				...
				<tbody>
					<?php if ( empty( $rows ) ) : ?>
						<tr><td colspan="8"><?php esc_html_e( 'No requests found.', '6arshid-social-community-main' ); ?></td></tr>
					<?php else : ?>
									...
									<?php if ( ! empty( $doc_paths ) ) : ?>
										<?php foreach ( $doc_paths as $idx => $path ) : ?>
											<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=arshid6social_serve_verification_doc&request_id=' . $row->id . '&idx=' . $idx . '&nonce=' . wp_create_nonce( 'arshid6social_ajax_nonce' ) ) ); ?>" target="_blank">
												...
												echo esc_html( sprintf( __( 'Doc %d', '6arshid-social-community-main' ), $idx + 1 ) ); ?>
											</a><br>
										<?php endforeach; ?>
									<?php else : ?>
										—
								...
								<td class="actions column-actions<?php echo esc_attr( $col_class( 'actions' ) ); ?>">
									<?php if ( 'pending' === $row->status ) : ?>
										<a href="<?php echo esc_url( wp_nonce_url(
											...
											<?php esc_html_e( 'Approve', '6arshid-social-community-main' ); ?>
										</a>
										<a href="<?php echo esc_url( wp_nonce_url(
											admin_url( 'admin.php?page=arshid6social-verification&arshid6social_verify_action=reject&request_id=' . $row->id ),
											'arshid6social_verify_action_' . $row->id
										) ); ?>" class="button button-small"
											onclick="return confirm('<?php esc_attr_e( 'Reject this request?', '6arshid-social-community-main' ); ?>')">
											<?php esc_html_e( 'Reject', '6arshid-social-community-main' ); ?>
										</a>
									<?php elseif ( 'approved' === $row->status ) : ?>
										...
										<a href="<?php echo esc_url( wp_nonce_url(
											...
											onclick="return confirm('<?php esc_attr_e( 'Revoke this verification badge?', '6arshid-social-community-main' ); ?>')">
											<?php esc_html_e( 'Revoke Badge', '6arshid-social-community-main' ); ?>
										</a>
