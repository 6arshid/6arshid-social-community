...
final class Admin_Marketplace {
	...
	public function setup_screen_options(): void {
		add_screen_option(
			'per_page',
			array(
				'label'   => __( 'Listings per page', '6arshid-social-community-main' ),
				'default' => 20,
	...
	public function get_columns(): array {
		return array(
			...
			'title'          => __( 'Title', '6arshid-social-community-main' ),
			'seller'         => __( 'Seller', '6arshid-social-community-main' ),
			'category'       => __( 'Category', '6arshid-social-community-main' ),
			'price'          => __( 'Price', '6arshid-social-community-main' ),
			'item_condition' => __( 'Condition', '6arshid-social-community-main' ),
			'location'       => __( 'Location', '6arshid-social-community-main' ),
	...
	public function handle_actions(): void {
		...
		if ( ! empty( $_GET['action'] ) && ! empty( $_GET['listing_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			...
			if ( ! check_admin_referer( 'arshid6social_mkt_' . $action . '_' . $listing_id ) ) {
				wp_die( esc_html__( 'Security check failed.', '6arshid-social-community-main' ) );
			}
	...
	public function render(): void {
		...
		// ── Helpers ───────────────────────────────────────────────────────────
		$status_labels = array(
			'active'   => __( 'Active', '6arshid-social-community-main' ),
			'draft'    => __( 'Draft', '6arshid-social-community-main' ),
			'archived' => __( 'Archived', '6arshid-social-community-main' ),
			'expired'  => __( 'Expired', '6arshid-social-community-main' ),
		);
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Products Archive', '6arshid-social-community-main' ); ?></h1>
			<hr class="wp-header-end">

			<?php // ── Status tabs ──────────────────────────────────────────── ?>
			<ul class="subsubsub">
				<?php
				$tab_items = array( '' => __( 'All', '6arshid-social-community-main' ) ) + $status_labels;
				$tab_list  = array();
			...
			<form method="get" style="margin-top:8px;">
				...
				<?php endif; ?>
				<p class="search-box">
					<label class="screen-reader-text" for="arshid6social-mkt-search"><?php esc_html_e( 'Search listings', '6arshid-social-community-main' ); ?></label>
					<input type="search" id="arshid6social-mkt-search" name="s" value="<?php echo esc_attr( $search ); ?>"
						placeholder="<?php esc_attr_e( 'Search by title, seller…', '6arshid-social-community-main' ); ?>" />
					<?php submit_button( __( 'Search Listings', '6arshid-social-community-main' ), 'secondary', '', false ); ?>
				</p>
			</form>
				...
				<table class="wp-list-table widefat fixed striped posts">
					<thead>
						<tr>
							...
							<th class="manage-column column-title column-primary"><?php esc_html_e( 'Title', '6arshid-social-community-main' ); ?></th>
							<th class="manage-column column-seller<?php echo esc_attr( $col_class( 'seller' ) ); ?>"><?php esc_html_e( 'Seller', '6arshid-social-community-main' ); ?></th>
							<th class="manage-column column-category<?php echo esc_attr( $col_class( 'category' ) ); ?>"><?php esc_html_e( 'Category', '6arshid-social-community-main' ); ?></th>
							<th class="manage-column column-price<?php echo esc_attr( $col_class( 'price' ) ); ?>"><?php esc_html_e( 'Price', '6arshid-social-community-main' ); ?></th>
							<th class="manage-column column-item_condition<?php echo esc_attr( $col_class( 'item_condition' ) ); ?>"><?php esc_html_e( 'Condition', '6arshid-social-community-main' ); ?></th>
							<th class="manage-column column-location<?php echo esc_attr( $col_class( 'location' ) ); ?>"><?php esc_html_e( 'Location', '6arshid-social-community-main' ); ?></th>
					...
					<tbody>
						<?php if ( empty( $listings ) ) : ?>
							<tr>
								<td colspan="9"><?php esc_html_e( 'No listings found.', '6arshid-social-community-main' ); ?></td>
							</tr>
						<?php else : ?>
							<?php foreach ( $listings as $listing ) : ?>
								...
								$price    = $listing->is_free
									? __( 'Free', '6arshid-social-community-main' )
								...
								<tr>
									...
									<td class="title column-title column-primary has-row-actions">
										<strong><?php echo esc_html( $listing->title ?: __( '(no title)', '6arshid-social-community-main' ) ); ?></strong>
										<div class="row-actions">
											<?php if ( 'active' !== $listing->status ) : ?>
												<span class="activate">
													<a href="<?php echo esc_url( $activate_url ); ?>"><?php esc_html_e( 'Activate', '6arshid-social-community-main' ); ?></a> |
												</span>
											<?php endif; ?>
											<?php if ( 'archived' !== $listing->status ) : ?>
												<span class="trash">
													<a href="<?php echo esc_url( $archive_url ); ?>"><?php esc_html_e( 'Archive', '6arshid-social-community-main' ); ?></a> |
												</span>
											<?php endif; ?>
											<span class="delete">
												<a href="<?php echo esc_url( $delete_url ); ?>"
													class="submitdelete"
													onclick="return confirm('<?php esc_attr_e( 'Delete this listing permanently?', '6arshid-social-community-main' ); ?>')">
									...
									<td class="seller column-seller<?php echo esc_attr( $col_class( 'seller' ) ); ?>">
										...
										<?php else : ?>
											<span style="color:#999;"><?php esc_html_e( 'Unknown', '6arshid-social-community-main' ); ?></span>
										<?php endif; ?>
									</td>
		...
		$mkt_admin_js = '(function(){
			...
			window.arshid6social_mkt_confirm_bulk = function(btn) {
				...
				if ( ! action ) {
					alert(' . wp_json_encode( __( 'Please select a bulk action.', '6arshid-social-community-main' ) ) . ');
					return false;
				}
				var checked = form.querySelectorAll("input[name=\'listing_ids[]\']:checked");
				if ( ! checked.length ) {
					alert(' . wp_json_encode( __( 'Please select at least one listing.', '6arshid-social-community-main' ) ) . ');
					return false;
				}
				if ( action === "delete" ) {
					return confirm(' . wp_json_encode( __( 'Delete selected listings permanently?', '6arshid-social-community-main' ) ) . ');
				}
				return true;
			};
		})();';
		...
		<div class="tablenav <?php echo esc_attr( $position ); ?>">
			<div class="alignleft actions bulkactions">
				<label class="screen-reader-text" for="bulk-action-selector-<?php echo esc_attr( $position ); ?>">
					<?php esc_html_e( 'Select bulk action', '6arshid-social-community-main' ); ?>
				</label>
				<select name="action" id="bulk-action-selector-<?php echo esc_attr( $position ); ?>">
					<option value=""><?php esc_html_e( 'Bulk actions', '6arshid-social-community-main' ); ?></option>
					<option value="set_active"><?php esc_html_e( 'Set Active', '6arshid-social-community-main' ); ?></option>
					<option value="set_archived"><?php esc_html_e( 'Set Archived', '6arshid-social-community-main' ); ?></option>
					<option value="delete"><?php esc_html_e( 'Delete', '6arshid-social-community-main' ); ?></option>
				</select>
				<input type="submit" class="button action"
					value="<?php esc_attr_e( 'Apply', '6arshid-social-community-main' ); ?>"
