	...
	public function render(): void {
		...
		<div class="wrap arshid6social-admin-notif">
			<h1 class="wp-heading-inline">
				<?php esc_html_e( 'Notifications', '6arshid-social-community-main' ); ?>
			</h1>
			...
			<div class="arshid6social-admin-notif-stats">
				<div class="arshid6social-stat-card">
					...
					<p><?php esc_html_e( 'Total Notifications', '6arshid-social-community-main' ); ?></p>
				</div>
				<div class="arshid6social-stat-card">
					<span style="font-size:2rem;">🔴</span>
					<strong><?php echo esc_html( number_format_i18n( $unread_notifs ) ); ?></strong>
					<p><?php esc_html_e( 'Unread', '6arshid-social-community-main' ); ?></p>
				</div>
				<div class="arshid6social-stat-card">
					...
					<p><?php esc_html_e( 'Members with unread', '6arshid-social-community-main' ); ?></p>
				</div>
			</div>
			...
			<div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;margin-top:2rem;align-items:start;">
				...
				<div class="postbox">
					<div class="postbox-header">
						<h2><?php esc_html_e( 'Notification Types', '6arshid-social-community-main' ); ?></h2>
					</div>
					<div class="inside">
						<p class="description" style="margin-bottom:1rem;">
							<?php esc_html_e( 'Disable a type to stop generating those notifications site-wide. Existing notifications are not deleted.', '6arshid-social-community-main' ); ?>
						</p>
						<form id="arshid6social-admin-notif-types-form">
							<table class="widefat striped" style="border:none;">
								<thead>
									<tr>
										...
										<th style="text-align:center;"><?php esc_html_e( 'Total sent', '6arshid-social-community-main' ); ?></th>
										<th style="text-align:center;"><?php esc_html_e( 'Enabled', '6arshid-social-community-main' ); ?></th>
									</tr>
								</thead>
								<tbody>
									<?php
							...
							<p style="margin-top:1rem;">
								<button type="submit" class="button button-primary">
									<?php esc_html_e( 'Save Type Settings', '6arshid-social-community-main' ); ?>
								</button>
				...
				<div class="postbox">
					<div class="postbox-header">
						<h2><?php esc_html_e( 'Email Settings', '6arshid-social-community-main' ); ?></h2>
					</div>
					<div class="inside">
						<form method="post" action="options.php">
							...
							<table class="form-table" role="presentation">
								...
								<tr>
									<th scope="row"><?php esc_html_e( 'Default Email Digest', '6arshid-social-community-main' ); ?></th>
									<td>
										<select name="arshid6social_email_digest">
											<option value="none"   <?php selected( get_option( 'arshid6social_email_digest', 'daily' ), 'none' ); ?>><?php esc_html_e( 'Never', '6arshid-social-community-main' ); ?></option>
											<option value="daily"  <?php selected( get_option( 'arshid6social_email_digest', 'daily' ), 'daily' ); ?>><?php esc_html_e( 'Daily digest', '6arshid-social-community-main' ); ?></option>
											<option value="weekly" <?php selected( get_option( 'arshid6social_email_digest', 'daily' ), 'weekly' ); ?>><?php esc_html_e( 'Weekly digest', '6arshid-social-community-main' ); ?></option>
										</select>
										<p class="description"><?php esc_html_e( 'Members can override this per their own profile settings.', '6arshid-social-community-main' ); ?></p>
									</td>
								</tr>
							</table>
							<?php submit_button( __( 'Save Email Settings', '6arshid-social-community-main' ) ); ?>
						</form>
					</div>
				</div>
			...
			<div class="postbox" style="margin-top:2rem;">
				<div class="postbox-header" style="display:flex;align-items:center;justify-content:space-between;">
					<h2><?php esc_html_e( 'Recent Notifications', '6arshid-social-community-main' ); ?></h2>
					<div style="padding:.75rem 1rem;">
						<button id="arshid6social-admin-delete-all" class="button button-link-delete">
							<?php esc_html_e( 'Delete all notifications', '6arshid-social-community-main' ); ?>
						</button>
					</div>
				</div>
				<div class="inside" style="padding:0;">
					<?php if ( empty( $recent ) ) : ?>
						<p style="padding:1.5rem;"><?php esc_html_e( 'No notifications found.', '6arshid-social-community-main' ); ?></p>
					<?php else : ?>
					<table class="wp-list-table widefat fixed striped" id="arshid6social-notif-log">
						<thead>
							<tr>
								...
								<th><?php esc_html_e( 'Recipient', '6arshid-social-community-main' ); ?></th>
								<th><?php esc_html_e( 'From', '6arshid-social-community-main' ); ?></th>
								<th><?php esc_html_e( 'Type', '6arshid-social-community-main' ); ?></th>
						...
						<tbody>
							...
							<tr data-id="<?php echo esc_attr( $notif->id ); ?>">
								...
								<td>
									<?php if ( $notif->is_new ) : ?>
										<span style="color:#dc2626;font-weight:600;"><?php esc_html_e( 'Unread', '6arshid-social-community-main' ); ?></span>
									<?php else : ?>
										<span style="color:#16a34a;"><?php esc_html_e( 'Read', '6arshid-social-community-main' ); ?></span>
									<?php endif; ?>
								</td>
					...
					<div style="padding:.75rem 1rem;border-top:1px solid #ddd;display:flex;align-items:center;gap:.75rem;">
						<button id="arshid6social-admin-delete-selected" class="button button-link-delete" disabled>
							<?php esc_html_e( 'Delete selected', '6arshid-social-community-main' ); ?>
						</button>
		...
		$notif_js = '(function() {
			...
			document.addEventListener("click", async function(e) {
				...
				if (!btn) return;
				if (!confirm(' . wp_json_encode( __( 'Delete this notification?', '6arshid-social-community-main' ) ) . ')) return;
			...
			document.getElementById("arshid6social-admin-delete-selected")?.addEventListener("click", async function() {
				...
				if (!ids.length || !confirm(' . wp_json_encode( __( 'Delete selected notifications?', '6arshid-social-community-main' ) ) . ')) return;
			...
			document.getElementById("arshid6social-admin-delete-all")?.addEventListener("click", async function() {
				if (!confirm(' . wp_json_encode( __( 'Delete ALL notifications? This cannot be undone.', '6arshid-social-community-main' ) ) . ')) return;
				const body = new FormData();
				body.append("action", "arshid6social_admin_delete_notifications");
				body.append("nonce", nonce);
				...
				if (res.success) {
					...
					document.getElementById("arshid6social-admin-log-msg").textContent = ' . wp_json_encode( __( 'All notifications deleted.', '6arshid-social-community-main' ) ) . ';
				}
			});
		})();';
