...
final class Sample_Data {
	...
	public function ajax_import(): void {
		...
		if ( get_option( 'arshid6social_sample_data_imported' ) ) {
			wp_send_json_error( array( 'message' => __( 'Sample data already exists. Delete it first.', 'social-network-6' ) ) );
		}
		...
		wp_send_json_success( array(
			'message' => sprintf(
				/* translators: counts */
				__( 'Imported: %1$d users, %2$d activities, %3$d notifications, %4$d listings, %5$d groups, %6$d bookmarks, %7$d messages, %8$d stories, %9$d ad.', 'social-network-6' ),
				count( $ids['users'] ),
	...
	public function ajax_delete(): void {
		...
		if ( ! get_option( 'arshid6social_sample_data_imported' ) ) {
			wp_send_json_error( array( 'message' => __( 'No sample data found to delete.', 'social-network-6' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Sample data deleted successfully.', 'social-network-6' ) ) );
	}
